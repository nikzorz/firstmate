...
- A `stalled` crew is Underway too, written as live work that stopped advancing and needs firstmate's attention rather than as work progressing on its own: the self-progressing tiebreak must not demote it to Charted Next, and like any validating task it stays out of Captain's Call unless only the captain can clear it.
