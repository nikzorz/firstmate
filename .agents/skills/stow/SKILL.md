...
- A project's existing committed project-memory file (`AGENTS.md`, or `CLAUDE.md` on a `+keep-claude-md` project), for project-intrinsic knowledge useful to nearly every session of that project, through a normal crewmate ship task following the project-memory instruction in its generated brief and the project's registered delivery mode.
...
1. Reduce non-pinned material now.
   ...
   Autonomously relocate it only by adding it to an already-existing allowed JIT note, or by routing it through a project's established delivery path to its existing owning project-memory file, then confirming that destination holds the quoted entry before removing the memory entry.
...
3. **Write within the existing boundaries.**
   ...
   - Project-intrinsic knowledge never goes directly into a project's project-memory file.
     Route it through a normal ship task so a crewmate records it through the project-memory instruction in its generated brief and the project's delivery path.
