...
- Never add an agent name as a commit co-author.
  ...
  Three things hold the line without you: `bin/fm-spawn.sh` launches every claude worker with attribution off, `.claude/settings.json` does the same for claude sessions and pipeline agents running in this repo, and `bin/fm-pr-merge.sh` supplies the squash message so a trailer that reached a branch commit cannot reach the default branch.
  None of them reaches codex, whose attribution is an account-side workspace setting that overrides repository rules, and the strip recognises only the claude and codex addresses that were measured, so do not add the trailer by hand on any harness.
