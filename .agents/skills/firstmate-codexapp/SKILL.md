...
The current supported shape is Desktop host-tool choreography plus explicit checks that the thread read the brief it was pointed at and wrote its status file, not a `codex-app` value in `FM_BACKEND`.
...
4. Decide whether this is a real Firstmate-managed task or a visible companion thread.
   A real task needs a task id, an isolated worktree or Desktop-owned cwd, a branch plan, a readable launch brief, and a writable `state/<id>.status` path.
...
For a Firstmate-managed task, do not restate any part of the crewmate contract in the thread message.
The brief `bin/fm-brief.sh` scaffolds is the single owner of that contract: the task, the setup, every rule, the status protocol, and the definition of done the task's delivery mode requires.
An excerpt leaves the brief's internal cross-references dangling past its boundary, so point at the brief instead of copying it, and hand over absolute paths rather than describing where a file lives.

Point the thread at a launch brief, never at `brief.md` alone.
A spawned worker receives `data/<task-id>/launch-brief.md`, which puts the worker role contract (`fm_brief_worker_role` in `bin/fm-dod-lib.sh`: what the worker is, which steering inbox it owns, and that a Firstmate task follows its brief rather than the repository's supervisor contract) ahead of the brief, plus the intent overlay for a no-mistakes ship.
A Desktop thread never passes through `bin/fm-spawn.sh`, so render that same launch brief at `<absolute-firstmate-home>/data/<task-id>/launch-brief.md` following the overlay that script's header describes before sending anything.
...
Send both absolute paths with only the context the brief cannot know:

```text
Your brief is <absolute-firstmate-home>/data/<task-id>/launch-brief.md and your status file is <absolute-firstmate-home>/state/<task-id>.status.
...
Never leave a thread running on a brief it could not read: without that file it has no role, no task, no rules, no status protocol, and no definition of done.
