...
Bootstrap reports successful AGENTS.md re-read sends as `BOOTSTRAP_INFO:` and emits `NUDGE_SECONDMATES:` only when that send fails, or when its delivery could not be confirmed, and the nudge needs retry.
An unconfirmed nudge reads `send unconfirmed:` rather than `send failed:` because it may already have landed; [`bootstrap-diagnostics`](../bootstrap-diagnostics/SKILL.md) owns how to handle each line.
