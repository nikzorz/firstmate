...
When a child's records cannot be attributed and cleared safely, retirement refuses ahead of every destructive step and the home keeps all of them for hand inspection; a child meta that lands mid-retirement still refuses before the home is given up, but only after the secondmate's own endpoint is gone.
...
Ahead of the child sweep and every kill, teardown also scans the retiring home for a process that detached from whatever started it, and scans each child's own home, or each non-Orca child's worktree, before closing that child's window.
...
That scan leaves out the sessions the home's own process-event runners live in, because the process-event retirement above is the owner that ends them.
