...
A pointer that landed but whose pending-reply bookkeeping write failed clears its marker, so it is never redelivered, and reports `delivered, bookkeeping incomplete` because the durable state behind that send needs a human.
