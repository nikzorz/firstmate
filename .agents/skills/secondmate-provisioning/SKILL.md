...
Bootstrap reports every AGENTS.md re-read send that landed as `BOOTSTRAP_INFO:`, and adds a `NUDGE_SECONDMATES:` line when the send failed, when its delivery could not be confirmed, or when it landed with its bookkeeping write incomplete.
An unconfirmed nudge reads `send unconfirmed:` rather than `send failed:` because it may already have landed, while a nudge that landed is never retried; [`bootstrap-diagnostics`](../bootstrap-diagnostics/SKILL.md) owns how to handle each line.
...
A send whose delivery `fm-send` could not confirm keeps the same retry marker but reports `send unconfirmed` rather than `send failed`, because the pointer may already have landed and re-reading one costs nothing.
A pointer that landed but whose pending-reply bookkeeping write failed clears its marker, so it is never redelivered, and reports `delivered, bookkeeping incomplete` with the reason that says which write failed and whether the watcher reconciles it unaided.
