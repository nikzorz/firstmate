...
- Helper scripts in `bin/` are plain bash.
  ...
  Each must also answer `--help` and `-h` with that header before it validates any argument, so a helper never reads the flag as an identifier; `tests/fm-help-flag.test.sh` enforces it and owns the short exemption list.
...
Commits here never name an agent as a co-author, and both harnesses whose behavior has been measured, claude and codex, are instructed by their own vendor to add one anyway.
...
The strip recognises only those two vendors' addresses; on any other harness, check your own commits, because nothing here has measured what it injects.
