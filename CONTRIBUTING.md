...
`bin/fm-spawn.sh` launches every claude worker with attribution off, `.claude/settings.json` does the same for Claude Code inside this repo, and `bin/fm-pr-merge.sh` supplies the squash commit message so a trailer that still reached a branch commit is dropped before the default branch sees it.
