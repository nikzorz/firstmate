...
# Usage: fm-brief.sh <task-id> <repo-name> --mode <no-mistakes|direct-PR|local-only> [--branch-prefix <prefix>] [--forge <none|gerrit> [--shape squash]] [--project-memory <agents-md|keep-claude-md>] [--herdr-lab]
#        fm-brief.sh <task-id> <repo-name> --scout [--herdr-lab]
#        fm-brief.sh <task-id> --secondmate {<project>...|--no-projects}
#   A missing required argument prints this usage and names the argument, before
#   any other validation and before anything is written.
...
# --project-memory selects that section's posture and defaults to agents-md.
# keep-claude-md is the registry's +keep-claude-md flag (bin/fm-project-mode.sh's
# header owns it and its --project-memory query), resolved by firstmate at
# intake like the forge; this script never looks it up. It replaces the section
# with the matching prohibition rather than dropping it, because an omitted
# instruction would still leave the worker free to create the AGENTS.md that the
# project's recorded captain decision forbids, and it opens with the fixed
# machine-readable "Project memory posture: keep-claude-md" line that
# bin/fm-spawn.sh checks against the registry, as it does the forge. Refused on
# --scout and --secondmate, which render no project-memory section.
...
# A missing required argument must name the argument rather than abort on an
...
for a in "$@"; do
  if [ -n "$want_value" ]; then
    ...
    case "$want_value" in
      ...
      project-memory) PROJECT_MEMORY=$a; PROJECT_MEMORY_SET=1 ;;
  ...
  case "$a" in
    ...
    --project-memory) want_value="project-memory" ;;
    --project-memory=*) PROJECT_MEMORY=${a#--project-memory=}; PROJECT_MEMORY_SET=1 ;;
...
# The positional count is checked first, so a bare invocation names the missing
# task id rather than a missing --mode.
...
if [ "$KIND" = ship ]; then
  case "$PROJECT_MEMORY" in
    agents-md|keep-claude-md) ;;
    *) echo "error: --project-memory must be agents-md or keep-claude-md (got '$PROJECT_MEMORY')" >&2; exit 1 ;;
  esac
elif [ "$PROJECT_MEMORY_SET" -eq 1 ]; then
  echo "error: --project-memory applies only to ship briefs; a scout and a secondmate charter render no project-memory section" >&2
  exit 1
fi
ID=${POS[0]}
...
# The charter's project list is validated before anything is written, so a
# refusal never leaves a half-created brief directory behind.
if [ "$KIND" = secondmate ]; then
  SECONDMATE_PROJECTS=""
  idx=1
  while [ "$idx" -lt "${#POS[@]}" ]; do
  ...
  if [ "$NO_PROJECTS" -eq 1 ]; then
    ...
    [ -n "$SECONDMATE_PROJECTS" ] || die_missing "<project> (--secondmate requires at least one project, or --no-projects for a project-less home)"
  fi
fi
...
# The project-memory instruction is generated, never omitted (see the header).
# The keep-claude-md variant carries no existence trigger: the default one fires
# a helper run that this posture forbids, so it would condition nothing.
case "$PROJECT_MEMORY" in
  keep-claude-md)
    IFS= read -r -d '' PROJECT_MEMORY_SECTION <<EOF || true
# Project memory
Project memory posture: keep-claude-md
