...
#
# On a GitHub squash merge this script supplies the commit message rather than
# letting the forge compose one, because a composed squash message carries every
# agent attribution trailer the branch commits carry and hoists them into its own
...
# the lines bin/fm-attribution-lib.sh recognises as agent attribution removed; it
# is passed as --subject and --body-file. The headline and body are asked for
# separately so each arrives as raw text with no delimiter to guess at. A default
# that cannot be read stops the merge rather than falling back to a composed
# message, which is the case this exists to prevent.
# Ownership of that message splits by half, because the trailers live in the
# body and never in the subject: a caller's --body or --body-file (-b, -F) takes
# the body and the strip stands down with it, while a caller's --subject (-t)
# takes only the subject and the stripped body is still supplied underneath it.
# An --auto request under --attended-override is refused wherever this would
# supply the message, because the forge stores the message when auto-merge is
# armed and lands it whenever the merge later fires, so commits pushed in between
# would be missing from it; a caller who owns the body is unaffected. The
# guarantee is squash-only by construction: a merge-commit or rebase merge
# replays the branch commits untouched, trailers included. A GitLab merge is left
# alone, because its message is the project's own setting.
#
# A GitHub merge also runs an ADVISORY closing-keyword check that blocks
# nothing. It reads the issue URLs on the task's backlog row (the links: entry
# tasks-axi derives from them) and the pull request body from the same live view
# the pre-merge conditions use, and warns, naming each issue, when no closing
# keyword immediately precedes a reference to it: GitHub acts only on that form,
...
# the body could not be read) is read back, and one not yet observed closed is
# reported with a hedge, because the forge closes on a keyword in a background
# job that can finish after that read. A queued or refused merge reads nothing
# back.
#
...
MERGE_CONTROL_LOCK=
MERGE_META_LOCK=
# Scratch files live in the one EXIT trap below, because a second trap would
# replace it and leak the locks it releases.
MERGE_SCRATCH_FILES=()
merge_control_cleanup() {
  ...
  [ "${#MERGE_SCRATCH_FILES[@]}" -eq 0 ] || rm -f -- "${MERGE_SCRATCH_FILES[@]}"
}
trap merge_control_cleanup EXIT
merge_scratch_file() {  # <var-name> <tag>
  local path
  path=$(mktemp "${TMPDIR:-/tmp}/fm-pr-merge-$2.XXXXXX") || return 1
  MERGE_SCRATCH_FILES+=("$path")
  printf -v "$1" '%s' "$path"
}
...
github_verify_mergeable() {
  ...
  if ! json=$(gh pr view "$URL" --json state,isDraft,mergeable,mergeStateStatus,headRefOid,baseRefName,statusCheckRollup,body 2>/dev/null) \
    || [ -z "$json" ]; then
...
# A caller who writes the squash body owns it, including its trailers. gh also
# takes -b and -F, alone or ending a short-option cluster.
...
# The refusal stays fixed; only its cause comes from gh, which writes a one-line
# summary to stderr.
GH_STDERR_FILE=
refuse_unreadable_default_message() {
  echo "error: the default squash message could not be read; nothing was handed to the forge" >&2
...
# Appends the stripped default squash message to merge_args (see the header).
# Owner and repository are String! variables, so they go through gh's raw field
# flag: the typed one turns an all-digit name into a JSON number, which the
# server refuses to coerce. A pull request the query cannot resolve, and a merge
# text the forge declines to compute, both come back as the literal token "null"
# with a zero exit, which is an unreadable default rather than a message.
github_supply_squash_message() {
  local query subject body body_file
  if [ -n "$FM_PR_GITHUB_CALLER_METHOD" ] && ! github_caller_method_is squash; then
    return 0
  fi
  caller_writes_squash_body "$@" && return 0
  ...
  if ! merge_scratch_file GH_STDERR_FILE gh-stderr || ! merge_scratch_file body_file squash-body; then
    echo "error: could not create a scratch file for the squash message; nothing was handed to the forge" >&2
    return 1
  fi
  # shellcheck disable=SC2016  # GraphQL variables, not shell expansions.
  query='query($owner:String!,$repo:String!,$number:Int!){repository(owner:$owner,name:$repo){pullRequest(number:$number){viewerMergeHeadlineText(mergeType:SQUASH) viewerMergeBodyText(mergeType:SQUASH)}}}'
  if ! caller_writes_squash_subject "$@"; then
    if ! subject=$(gh api graphql -f query="$query" \
      -f owner="$PR_OWNER" -f repo="$PR_REPO" -F number="$PR_NUMBER" \
      --jq '.data.repository.pullRequest.viewerMergeHeadlineText' 2>"$GH_STDERR_FILE") \
      || [ -z "$subject" ] || [ "$subject" = null ]; then
      refuse_unreadable_default_message
      return 1
    fi
  ...
  if ! body=$(gh api graphql -f query="$query" \
    -f owner="$PR_OWNER" -f repo="$PR_REPO" -F number="$PR_NUMBER" \
  ...
  if ! printf '%s\n' "$body" | fm_attribution_strip > "$body_file"; then
    echo "error: could not write the squash message; nothing was handed to the forge" >&2
    return 1
  fi
...
read_task_issue_urls() {
  ...
  if ! fm_backlog_tasks_axi_addressing "$data" 2>/dev/null; then
    CLOSING_CHECK_ERROR=${FM_BACKLOG_TRANSITION_ERROR:-the backlog backend could not be resolved}
    return 2
  fi
  if [ -n "$FM_BACKLOG_AXI_FILE" ] && [ ! -e "$FM_BACKLOG_AXI_FILE" ] && [ ! -L "$FM_BACKLOG_AXI_FILE" ]; then
    return 1
  fi
  out=$(fm_backlog_row_show "$data" "$ID" --full 2>&1) || status=$?
  if [ "$status" -ne 0 ]; then
    if printf '%s\n' "$out" | grep -q '^code: NOT_FOUND$'; then
      return 1
    fi
    CLOSING_CHECK_ERROR=$(printf '%s\n' "$out" | sed -n '1p')
...
# A bare "#N" resolves against the pull request's own repository, so it names an
# issue only there. GitHub resolves owner and repository case-insensitively, so a
# hand-typed "Firstmate" on a backlog row is the same repository as "firstmate"
# in the pull request URL.
issue_is_in_pr_repo() {  # <owner> <repo>
  [ "$(printf '%s' "$1/$2" | LC_ALL=C tr '[:upper:]' '[:lower:]')" \
    = "$(printf '%s' "$PR_OWNER/$PR_REPO" | LC_ALL=C tr '[:upper:]' '[:lower:]')" ]
}

# The reference a closing keyword must immediately precede, in the spelling this
# pull request's repository makes valid.
...
# that keeps "#1" from matching inside "#14", and keeps "#117x" from reading as a
# reference to 117, which GitHub parses as no reference at all.
...
# Advisory only (see the header): every answer it reaches is a report.
closing_keyword_gate() {
  local url owner repo number ref rc=0 body_file
  local -a issues=() missing=() closing=()
  read_task_issue_urls || rc=$?
  case "$rc" in
    0) ;;
    1) return 0 ;;
    *) closing_check_did_not_run "$CLOSING_CHECK_ERROR"; return 0 ;;
  esac
...
URLS
  ...
  if ! merge_scratch_file body_file pr-body; then
    closing_check_did_not_run "a scratch file for the pull request body could not be created"
    return 0
  fi
  if ! printf '%s' "$FM_PR_GITHUB_VIEW_JSON" \
    | jq -r 'if (.body | type) == "string" then .body else error("no body") end' \
    > "$body_file" 2>/dev/null; then
    closing_check_did_not_run "the pull request body could not be read"
    return 0
  fi
  ...
  # No edit is prescribed: the record cannot establish that this work owns the
  # issue, so only a reader can tell which of these two it is.
...
# The one check no keyword form can fool, run only after a verified landed merge.
report_unclosed_issues() {
  ...
  for url in "${VERIFY_ISSUE_URLS[@]}"; do
    ...
    case "$state" in
      ...
      *) echo "warning: $url was not observed closed when read just after the merge; the forge closes on a keyword in a background job that can finish after this read, so it may still close on its own" >&2 ;;
    esac
  done
}
...
case "$PROVIDER" in
  github)
    ...
    closing_keyword_gate
    github_supply_squash_message "$@" || exit 1
