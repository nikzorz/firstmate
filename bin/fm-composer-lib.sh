...
# baseline and the post-Enter transition this way, and also for a recorded
# harness with no verified signature of its own). cursor's `ctrl+c to stop` is
...
# fm_busy_regex_for_harness: the verified delivery-busy signature for
# <harness>, and the one place that knows which harnesses have one. An empty
# harness gets the generic union; a harness with no verified signature of its
# own gets nothing, because it must never borrow another harness's.
