...
# fm_tmux_submit_busy_harness: the harness a submit's busy reads are scoped to.
# A recorded harness with no verified signature of its own falls back to the
# generic union here, and only here: "not busy" is an assertion a submit
# confirmation has no basis for on such a pane, so it must not report a
# delivered message as a swallow. A verified harness keeps its own signature
# and never borrows another's.
fm_tmux_submit_busy_harness() {  # [harness]
  local harness=${1:-}
...
# The optional trailing <harness> scopes each busy read in both cores (the
# pre-typing baseline, the turn-started poll, and the queued-Enter check) to
# that harness's verified footer signature. Without it the generic union cannot see every verified
# harness's spinner (Claude's elapsed-time spinner is not in it), so a busy pane
# reads idle and a delivered message is reported as a swallow.
fm_tmux_submit_enter_core() {  # <target> <retries> <enter-sleep> [baseline-idle] [harness]
  local target=$1 retries=$2 sleep_s=$3 baseline_idle=${4:-} harness i=0 j state busy_state
  harness=$(fm_tmux_submit_busy_harness "${5:-}")
  while :; do
