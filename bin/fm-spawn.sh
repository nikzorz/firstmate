...
#   ship spawn also refuses a brief whose recorded project-memory posture
#   disagrees with the registered one (bin/fm-project-mode.sh --project-memory);
#   a brief with no project-memory section is not checked. A
...
#   `## Captain's intent` line opening with a Captain label or address. A line
#   that is only `{TASK}` anywhere in the brief counts as unfilled, so a legacy
#   `# Task` body or a leftover line under filled intent is refused too; prose
#   that merely names the placeholder is not.
...
if [ "$KIND" = ship ]; then
  ...
  # The registered project-memory posture (bin/fm-project-mode.sh) is a captain
  # decision about which file a project keeps its memory in, so a brief that
  # disagrees with it would tell the worker to create the AGENTS.md the project
  # forbids, or forbid the one it wants. A brief with no project-memory section
  # (a promoted scout) carries no instruction either way and is not checked.
  if grep -qx '# Project memory' "$BRIEF"; then
    BRIEF_MEMORY=$(sed -n 's/^Project memory posture: //p' "$BRIEF" | head -n 1)
    [ -n "$BRIEF_MEMORY" ] || BRIEF_MEMORY=agents-md
    STANDING_MEMORY=$("$FM_ROOT/bin/fm-project-mode.sh" --project-memory "$PROJ_NAME" 2>/dev/null) || STANDING_MEMORY=
    [ -n "$STANDING_MEMORY" ] || STANDING_MEMORY=agents-md
    if [ "$BRIEF_MEMORY" != "$STANDING_MEMORY" ]; then
      echo "error: project-memory mismatch for $ID: $PROJ_NAME is registered project-memory=$STANDING_MEMORY but $SOURCE_BRIEF records $BRIEF_MEMORY; keep the filled ## Captain's intent and ## Firstmate spec bodies, remove $SOURCE_BRIEF, re-scaffold it with --project-memory $STANDING_MEMORY, then re-fill those two subsections, so the worker's project-memory instruction matches the project's recorded decision" >&2
      exit 1
    fi
  fi
...
# the records being dropped. $T is the endpoint supervision keys on; Orca
# additionally records a separate window= handle, and fm_backend_target_of_meta
# falls back to it when no terminal= was recorded, so both spellings are claimed.
# A relaunch claims nothing: the same task continues on its own recorded
# endpoint, so the supervision history under that key and id is its own.
if [ "$RELAUNCH" -eq 0 ]; then
