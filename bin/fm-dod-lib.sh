...
# bin/fm-crew-state.sh alone composes a second gate after it,
# fm_dod_accept_direct_pr_done: a direct-PR worker is briefed to open its pull
# request before reporting done, so a direct-PR `done:` naming no pull request
# anywhere is a task still owed a steer, not a finished delivery.
...
# placeholder, or when any line of the brief is nothing but `{TASK}`: a legacy
# `# Task` body and a leftover line under filled intent are unfilled the same way,
# while prose that merely names the placeholder is not. A missing file carries none.
fm_brief_task_placeholders_present() {  # <file>
  local file=$1 intent spec
  [ -f "$file" ] || return 1
  grep -Eq '^[[:space:]]*\{TASK\}[[:space:]]*$' "$file" && return 0
...
# the pipeline, what `--intent` may carry, the two firstmate-specific rules, and
# how to end a run without losing its head.
...
If this task owns an issue, the PR body must close it, and the form is exact: the forge acts only when a closing keyword immediately precedes the reference.
...
# Where a PR/MR/change URL ENDS inside prose, one span per shape fm_pr_url_parse
# accepts. A bound, never a second opinion on validity: fm_pr_url_parse alone
# decides whether the span names a real one. Taking the matched span makes the
# wrapping prose irrelevant by construction (parentheses, a markdown link, a
# trailing comma, `/files`), with no list of marks to keep up to date.
FM_DOD_PR_URL_SPAN_RE='https://[A-Za-z0-9._~%-]+(/[A-Za-z0-9._~%-]+)*/(pull|-/merge_requests|\+)/[1-9][0-9]*'

# How many URL-bearing status lines the stream scan reads, counted from the END.
# It runs on every crew-state read of a direct-PR done, including each task of a
# fleet snapshot, so it is bounded; it keeps the tail because a direct-PR worker
# announces its pull request at the close of the stream.
FM_DOD_PR_SCAN_LINES_DEFAULT=64

# The steer a refused direct-PR done reports, leading with the action and short
# enough to survive the Bearings current-state detail budget.
FM_DOD_DIRECT_PR_DONE_STEER='steer it to push and open its PR: briefed to open one before done:, none yet'

# 0 when <prose> names a URL fm_pr_url_parse accepts. Each parse runs in a
# subshell so this read-only probe never clobbers FM_PR_* globals a caller holds.
fm_dod_prose_names_pr_url() {  # <prose>
  local spans candidate
  spans=$(printf '%s\n' "$1" | grep -Eo "$FM_DOD_PR_URL_SPAN_RE") || return 1
  while IFS= read -r candidate; do
    ( fm_pr_url_parse "$candidate" ) >/dev/null 2>&1 && return 0
  done <<EOF
$spans
EOF
  return 1
...
# 0 when a ship done: needs no direct-PR presence check or has a pull request:
# the recorded pr= in <meta>, then the done note (`PR <url>` first, then any URL
# in its prose), then the last URL-bearing lines of <status-log>, cheapest
# first. A worker that announced its pull request on an earlier event delivered
# too. 1 when a direct-PR done names none anywhere; stdout then holds the steer.
# Presence only: it never asks whether the pull request is open, green, or the
# worker's own, which would need the forge. Every other mode and kind passes:
# no-mistakes treats a pre-validation done as the pipeline handoff, and
# local-only and scout finish without a pull request.
fm_dod_accept_direct_pr_done() {  # <kind> <mode> <line> <meta> [<status-log>]
  local kind=$1 mode=$2 line=$3 meta=$4 log=${5:-} note url lines bound
  [ "$kind" = ship ] && [ "$mode" = direct-PR ] || return 0
  [ "$(status_line_verb "$line")" = "done" ] || return 0
  [ -z "$(fm_dod_meta_value "$meta" pr)" ] || return 0
  note=$(status_line_note "$line")
  if url=$(fm_dod_pr_url_from_done_note "$note") && ( fm_pr_url_parse "$url" ) >/dev/null 2>&1; then
    return 0
  fi
  fm_dod_prose_names_pr_url "$note" && return 0
  if [ -n "$log" ] && [ -f "$log" ]; then
    bound=${FM_DOD_PR_SCAN_LINES:-$FM_DOD_PR_SCAN_LINES_DEFAULT}
    case "$bound" in ''|*[!0-9]*|0) bound=$FM_DOD_PR_SCAN_LINES_DEFAULT ;; esac
    lines=$(grep -F '://' "$log" 2>/dev/null | tail -n "$bound") || lines=
    [ -n "$lines" ] && fm_dod_prose_names_pr_url "$lines" && return 0
  fi
  printf '%s\n' "$FM_DOD_DIRECT_PR_DONE_STEER"
  return 1
}
