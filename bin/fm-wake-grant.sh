#!/usr/bin/env bash
# Grant the supervision branch the durable wake rows it may handle, under the
# wake-queue lock: activate binds the branch owner, publish records the eligible
# sequences for one generation, and release or deactivate withdraws them.
#
# Usage: fm-wake-grant.sh activate PID GENERATION | publish GENERATION SEQUENCE... | release GENERATION | deactivate PID GENERATION
set -u
