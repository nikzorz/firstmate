...
family_for_basename() {
  case "$1" in
    ...
    fm-crew-state.test.sh|fm-decision-hold-lifecycle.test.sh|fm-episode-records-lib.test.sh|\
    fm-documentation-audiences.test.sh|fm-ensure-agents-md.test.sh|fm-grok-harness.test.sh|\
    fm-help-flag.test.sh|fm-kimi-harness.test.sh|fm-herdr-lab.test.sh|\
    fm-instruction-owners.test.sh|fm-lint.test.sh|\
    fm-install-herdr.test.sh|fm-limit-resume.test.sh|\
    fm-nm-test-contract.test.sh|fm-no-mistakes-ownership.test.sh|\
    ...
    fm-spawn-dispatch-profile.test.sh|fm-spawn-episode-records.test.sh|fm-spawn-worktree-settle.test.sh)
      printf '%s\n' backend-dispatch
      ;;
    fm-attribution-lib.test.sh|fm-pr-check-security.test.sh|fm-pr-merge.test.sh|\
...
families_for_changed_path() {
  ...
  case "$path" in
    ...
    bin/fm-tasks-axi-lib.sh)
      printf '%s\n' pure-contract-unit
      # The backlog read this lib owns decides what bin/fm-pr-merge.sh reports
      # before every task PR merge, so a change here re-runs that family too.
      printf '%s\n' pr-forge
      ;;
    bin/fm-lint.sh|bin/fm-install-shellcheck.sh|\
    bin/fm-brief.sh|bin/fm-ensure-agents-md.sh|bin/fm-crew-state.sh|\
    bin/fm-decision-hold.sh|bin/fm-supervision*|bin/fm-transition-lib.sh|\
    bin/fm-tmux-lib.sh|bin/fm-marker-lib.sh|bin/fm-operational-input.sh|\
