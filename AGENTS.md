...
config/keep-awake  optional Windows/WSL opt-in that keeps the machine awake while away mode is armed; LOCAL, gitignored; absent means off, and not inherited; see docs/configuration.md "Away-mode keep-awake"
...
data/                personal fleet records; LOCAL, gitignored as a whole
  ...
  gate-links/<origin>/<key>  recorded pairing between a captain-gated item and the worker gate that answers it; owned by bin/fm-decision-hold.sh
projects/            cloned repos; gitignored; READ-ONLY for you
state/               volatile runtime signals; gitignored
  <id>.status        appended by crewmates: "<state>: <note>" wake-event lines, not current-state truth
  <id>.turn-ended    touched by turn-end hooks
  <id>.grok-turnend-token   firstmate-owned grok hook registry token for the task
  <id>.kimi-turnend-token   firstmate-owned Kimi hook registry token for the task
  <id>.pause-recheck   one-shot epoch at which a declared pause is rechecked ahead of its fixed cadence; bin/fm-classify-lib.sh owns it
  <id>.park-sighting   last park episode this task was seen stopped at, with the crew's status content at that sighting; bin/fm-classify-lib.sh owns it
  ...
  .afk-keep-awake .afk-keep-awake.out   keep-awake holder record and its readiness output; owned by bin/fm-keep-awake.sh; never touch
  ...
  .hash-* .count-* .stale-* .stale-since-* .paused-* .wedge-escalations-* .advancing-resurfaced-* .advancing-absorbs-* .seen-* .hb-surfaced-* .last-* .heartbeat-streak   watcher internals; never touch
...
3. **Wake queue** - when locked, drains the durable wake queue and prints the raw records prominently as this turn's first work queue; bounded, clearly labeled annotations - the latest status event, plus any decision or blocker still open behind it - may follow a valid `signal` record but never replace it or current-state reconciliation, and a lapsed watcher chain still surfaces here via the same guard alarm.
...
That skill is a router: it answers the cross-harness questions directly and names one reference file per harness, so open the file for the harness actually in play before acting on any harness-specific procedure.
...
Load `stuck-crewmate-recovery` after a stale wake, looping or confused pane, answered-by-brief question, unresponsive worker, or a steer that failed or whose delivery could not be confirmed.
...
That bare form is correct for a single no-action event, but a long unbroken run of them reads as silence rather than health, so roughly every fourth reply in such a stretch replaces it with one concrete sentence of current fleet state: what is running, what is waiting and on whom, what landed.
That periodic sentence is the only permitted expansion of a no-action reply and stays correct even when the state it reports is unchanged from the previous one, because a state snapshot is not a progress claim, but it never licenses routine progress, empty polls, elapsed time, or no-change updates offered as progress in the replies around it.
...
When that captain-gated thread asks a question a live worker's own gate will also raise, record the pairing with `bin/fm-decision-hold.sh gate-link` so answering the gate reconciles the item instead of leaving it stale; `ask-user-authority` owns when to record and reconcile it, and teardown refuses while a recorded pairing survives.
...
- `bootstrap-diagnostics` - load whenever the session-start digest's bootstrap section prints an actionable diagnostic line (`MISSING:`, `MISSING_MANUAL:`, `BACKEND_INVALID:`, `NEEDS_GH_AUTH`, `TANGLE:`, `CREW_DISPATCH: invalid`, `FLEET_SYNC:`, `PR_CHECK_MIGRATION:`, `SECONDMATE_SYNC:`, `SECONDMATE_LIVENESS:`, `NUDGE_SECONDMATES:`, `CONFIG_REREAD:`, or `FMX:`); silence and `BOOTSTRAP_INFO:` need no load.
- `diagnostic-reasoning` - load before scoping a reported bug and before acting on a diagnostic report.
- `ask-user-authority` - load before deciding any ask-user finding, regardless of the project's `yolo` posture, and before recording or reconciling a captain-gated backlog item against a worker's gate.
- `harness-adapters` - load before spawning or recovering a crewmate or secondmate, handling a trust dialog, sending a harness-specific skill invocation, interrupting or exiting an agent, resuming an exited agent, or verifying a new harness adapter; its router names the per-harness reference file to open for a harness-specific procedure.
- `firstmate-orca` - load before switching to Orca, spawning or supervising Orca-backed work, smoke-testing Orca backend behavior, debugging Orca task state, or reconciling Orca-backed task metadata.
- `project-management` - load before adding, creating, removing, or initializing a project.
- `stuck-crewmate-recovery` - load when the session-start digest reports an ordinary direct report's endpoint dead or its metadata has no window, or after a stale wake, looping pane, repeated confusion, an answered-by-brief question, an unresponsive crewmate, or a steer that failed or whose delivery could not be confirmed.
