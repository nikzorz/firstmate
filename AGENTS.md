...
config/keep-awake  optional Windows/WSL opt-in that keeps the machine awake while a daemon-backed away or quiet posture is armed; LOCAL, gitignored; absent means off, and not inherited; see docs/configuration.md "Away-mode keep-awake"
...
state/               runtime records and signals; gitignored
  ...
  extra-usage.check.sh .extra-usage-steered  generated Claude extra-usage check shim, its .check-trust binding, and its episode record; present only after bin/fm-extra-usage.sh arm
  ...
  .hash-* .count-* .stale-* .stale-since-* .churn-since-* .paused-* .wedge-escalations-* .dead-reported-* .advancing-resurfaced-* .advancing-absorbs-* .writing-* .waiting-* .seen-* .hb-surfaced-* .last-* .heartbeat-streak   watcher internals; never touch
...
A fresh spawn refuses a task id carrying a dot, the reserved `x-` prefix, or the name of a home-level `state/` record (reserved in `bin/fm-pr-lib.sh`), while a relaunch and other commands on an existing task still accept such an id, because `state/<id>.<suffix>` is one flat namespace and a secondmate retirement clears its home's leftover task records by sweeping `state/<id>.*`.
...
When a ship task is meant to close a GitHub issue, also record the owned-issue line `bin/fm-pr-merge.sh` reads, and never for an issue the work only references.
...
A `stalled` run has stopped advancing and needs attention rather than waiting out, and an empty or unrecognised run status reads `unknown`, which is never evidence of a healthy run.
...
That bare form is correct for a single true no-op, but a long unbroken run of them reads as silence rather than health, so roughly every fourth reply in such a stretch replaces it with one concrete sentence of current fleet state: what is running, what is waiting and on whom, what landed.
That periodic sentence is the only permitted expansion of a no-op reply and stays correct even when the state it reports is unchanged from the previous one, because a state snapshot is not a progress claim, but it never licenses routine progress, empty polls, elapsed time, or no-change updates offered as progress in the replies around it.
...
- `bootstrap-diagnostics` - load whenever the session-start digest's bootstrap or network-checks section prints an actionable diagnostic line (`MISSING:`, `MISSING_MANUAL:`, `PRESENTATION_UNAVAILABLE:`, `BACKEND_INVALID:`, `NEEDS_GH_AUTH`, `TANGLE:`, `STARTUP_MEMORY_BUDGET:`, `CREW_DISPATCH: invalid`, `FLEET_SYNC:`, `NETWORK_CHECKS:`, `HOME_SUMMARY:`, `BACKLOG_RECONCILE:`, `SECONDMATE_SYNC:`, `SECONDMATE_LIVENESS:`, `SECONDMATE_HANDOFF:`, `NUDGE_SECONDMATES:`, `CONFIG_REREAD:`, or `FMX:`), or when `BOOTSTRAP_INFO:` says an interrupted backlog cleanup may have left an endpoint or local copy; silence and other `BOOTSTRAP_INFO:` facts need no load.
...
- `stuck-crewmate-recovery` - load when the session-start digest reports an ordinary direct report's endpoint dead or its metadata has no window, after a stale wake, looping pane, repeated confusion, an answered-by-brief question, an unresponsive crewmate, or a failed steer, and whenever a live worker reports its no-mistakes pipeline dead, unreachable, or timed out.
  Also load it before instructing a worker to end its validation run, and when a worker or the pipeline reports the run's preserved work missing or unrecoverable.
