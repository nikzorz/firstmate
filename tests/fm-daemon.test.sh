...
# Age a record's MTIME, which is what the daemon's _file_age throttles read, so a
# test can drive several throttle windows without sleeping through them.
backdate_record() {  # <file> <seconds-ago>
  fm_touch_epoch "$(( $(date +%s) - $2 ))" "$1"
}

# Put a claude crew's semantic busy record into the busy state the way its
# lifecycle hooks do, so stale_window_is_busy reads the pane as resumed.
mark_claude_busy() {  # <state> <id>
  local gen
  gen=$("$ROOT/bin/fm-busy-event.sh" arm "$1" "$2" --state idle) || return 1
...
test_cross_clear_leaves_no_watcher_advancing_count_for_the_next_episode() {
  ...
  pid=$!
  wait_for_exit "$pid" 100 || { kill "$pid" 2>/dev/null; wait "$pid" 2>/dev/null
...
test_housekeeping_resumed_pane_drops_its_absorb_records() {
  ...
  # The crew resumed: its busy record says so, so every trace of the absorb goes.
  mark_claude_busy "$state" adv-w5 || fail "could not record the resumed crew as busy"
...
test_housekeeping_absorbed_pane_resuming_inside_one_window_restarts_its_clock() {
  ...
  mark_claude_busy "$state" adv-w6 || fail "could not record the resumed crew as busy"
  PATH="$fakebin:$PATH" FM_FAKE_TMUX_WINDOW="$win" FM_FAKE_TMUX_CAPTURE="$busy" \
    FM_FAKE_TMUX_CURRENT_COMMAND=claude \
    FM_STATE_OVERRIDE="$state" FM_STALE_ESCALATE_SECS=240 FM_PAUSE_RESURFACE_SECS=3600 \
    housekeeping "$state"
