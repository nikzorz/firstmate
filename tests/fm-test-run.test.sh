...
init_changed_fixture_repo() {
  ...
  mkdir -p "$repo/.agents/skills/example/references" "$repo/.claude" \
    "$repo/.pi/extensions" "$repo/src"
  : >"$repo/.agents/skills/example/SKILL.md"
  : >"$repo/.agents/skills/example/references/example.md"
...
test_changed_dependency_selection_and_unmapped_failure() {
  ...
  printf '\n' >>"$repo/.agents/skills/example/references/example.md"
  set +e
  listed=$(cd "$repo" && bin/fm-test-run.sh --list --changed --base HEAD 2>"$tmp/ref_err")
  rc=$?
  set -e
  [ "$rc" -eq 0 ] \
    || fail "skill reference source must map to a family, got exit $rc: $(cat "$tmp/ref_err")"
  assert_contains "$listed" "tests/fm-captain-translation-contract.test.sh" \
    "skill reference source selects pure contract coverage"
