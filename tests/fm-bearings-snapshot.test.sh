...
# needs attention rather than work that ended. The captain's in-flight list takes
# a secondmate home's children from its active child work, so a fold that
# recognises only `working` makes exactly the crew the state was added to expose
# disappear from the one view the captain reads.
...
SH
  ...
  json=$(run "$home" "$fakebin" --json)
  printf '%s' "$json" | jq -e '
    (.in_flight | any(.id == "stalls/hung-child" and .state == "stalled"))
