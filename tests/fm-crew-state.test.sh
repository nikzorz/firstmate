...
#       failed.
#   (m) a run that stopped advancing reads stalled: an active step quiet past
#       its inactivity budget, or a ci step waiting on a re-run the forge says
#       cannot arrive; an empty or unrecognised run status reads unknown.
#   (n) a claude crew parked on the usage-limit prompt reads usage-limited
#       ahead of its run, with the quota window's verdict.
#   (o) a direct-PR done naming no pull request anywhere reads blocked on the
#       steer its mode calls for.
set -u
...
exit 0
SH
  # `gh pr view` serves the crew-state forge probe. It answers for EVERY case,
  # so a fixture carrying a pull-request url can never reach the real forge;
  # an empty answer is the "forge had no answer" shape.
  cat > "$fb/gh" <<'SH'
#!/usr/bin/env bash
set -u
case "${1:-} ${2:-}" in
...
SH
  ...
  # it. Serving FM_FAKE_QUOTA_JSON verbatim keeps these cases off the real account.
  cat > "$fb/quota-axi" <<'SH'
#!/usr/bin/env bash
set -u
...
exit 0
SH
  chmod +x "$fb/no-mistakes" "$fb/gh" "$fb/gh-axi" "$fb/glab" "$fb/gerrit-axi" "$fb/tmux" "$fb/herdr" "$fb/quota-axi"
  printf '%s\n' "$fb"
}
...
# A quota-axi --json snapshot (schema 5, the shape fm_quota_json_valid admits)
# whose claude row reports <percent-remaining> effective headroom, bounded by
      ...
      "quotaSemantics": {
        ...
        "effectiveAvailability": [
          { "scope": "all_models", "status": "known", "effectivePercentRemaining": $1,
            "boundedBy": ["five_hour"], "runway": { "status": "through_reset" } }
        ]
      },
      "windows": [
        { "id": "five_hour", "percentRemaining": $1, "resetsAt": null }
      ]
    }
  ]
}
...
# --- stalled-run fixtures ---------------------------------------------------

# A ci step spending its re-run wait: its fix rounds are spent and it asks for a
# check re-run of checks it has already seen.
...
# The ci monitor logs only when the checks state changes, so a green monitor
# waiting on a merge goes quiet by design and its last activity is still the
# green line. That line keeps it out of the budget while the ci log gave no
# marker of its own; a readable log is the fuller record and wins.
test_quiet_green_monitor_with_an_unread_log_is_not_stalled() {
  reset_fakes
  local d; d=$(new_case quiet-green-monitor)
  make_repo_on_branch "$d/wt" fm/feat-qgm
  make_fakebin "$d" >/dev/null
  fm_write_meta "$d/state/qgm.meta" "window=fm:fm-qgm" "worktree=$d/wt" "kind=ship"
  FM_FAKE_AXI_STATUS="$(run_ci_monitoring_quiet fm/feat-qgm \
    '"quiet 2h58m ago: log: all CI checks passed - still monitoring until merged or closed"')"
  FM_FAKE_CI_LOGS=""
  local out; out=$(run_crew_state "$d" qgm)
  assert_contains "$out" "state: working" "a green monitor waiting on a merge is not stalled"
  assert_not_contains "$out" "stopped advancing" "the green last activity keeps it out of the budget"

  FM_FAKE_CI_LOGS="CI checks running, waiting for results..."
  out=$(run_crew_state "$d" qgm)
  assert_contains "$out" "state: stalled" "a readable ci log that is not green outranks the last activity line"
  pass "a quiet green monitor with an unread ci log is not stalled"
}
...
test_active_step_columns_are_located_by_name() {
  ...
  # The layout without a status column, with a bare `quiet <duration>` cell and
  # an unprefixed recent one.
  FM_FAKE_AXI_STATUS="$(run_with_active_step_layout fm/feat-cols \
    'step,active_for,last_activity,agent_pid,round' \
    'review,42m8s,"quiet 31m2s",44121,"auto-fix 1/3"')"
  out=$(run_crew_state "$d" cols)
  assert_contains "$out" "state: stalled" "a bare quiet cell past the budget must stall"
  assert_contains "$out" "review step quiet 31m" "the bare quiet cell is measured"
  FM_FAKE_AXI_STATUS="$(run_with_active_step_layout fm/feat-cols \
    'step,active_for,last_activity,agent_pid,round' \
    'review,12m3s,8s,44121,"auto-fix 1/3"')"
  out=$(run_crew_state "$d" cols)
  assert_contains "$out" "state: working" "a recent unprefixed cell stays working"
...
# `pending` is an executing status in the pipeline's own vocabulary (a queued
# run), so it stays working rather than falling to the unrecognised-status arm.
test_pending_run_status_stays_working() {
  reset_fakes
  local d; d=$(new_case pending-status)
  make_repo_on_branch "$d/wt" fm/feat-pend
  make_fakebin "$d" >/dev/null
  fm_write_meta "$d/state/pend.meta" "window=fm:fm-pend" "worktree=$d/wt" "kind=ship"
  FM_FAKE_AXI_STATUS="$(run_with_active_step fm/feat-pend review unknown pending)"
  local out; out=$(run_crew_state "$d" pend)
  assert_contains "$out" "state: working" "a queued run is not an unrecognised status"
  assert_contains "$out" "validating (pending)" "the queued status is named"
  pass "a pending run status stays working"
}
...
test_the_forge_is_asked_nothing_it_need_not_answer() {
  ...
  FM_CREW_STATE_FORGE_PROBE=1
  out=$(FM_CREW_STATE_NO_FORGE=1 run_crew_state "$d" nask)
  assert_contains "$out" "state: working" "the no-forge cadence read behaves as if the probe did not exist"
  [ ! -s "$FM_FAKE_GH_CALLS" ] || fail "FM_CREW_STATE_NO_FORGE=1 still called the forge"
...
# --- direct-PR delivery gate -------------------------------------------------

# Every row reads the same fixture shape: a direct-PR ship whose named head is on
# the remote, so only the pull-request presence gate can refuse it.
make_direct_pr_done_case() {  # <name> <id> <mode> [<extra-meta>...]
  local name=$1 id=$2 mode=$3 d
  shift 3
  d=$(new_case "$name")
  make_repo_on_branch "$d/wt" "fm/$id"
  make_fakebin "$d" >/dev/null
  fm_write_meta "$d/state/$id.meta" \
    "window=fm:fm-$id" "worktree=$d/wt" "kind=ship" "mode=$mode" "harness=claude" "$@"
  arm_idle_record "$d/state" "$id"
  printf '%s\n' "$d"
}

test_direct_pr_done_gate_reads_the_pull_request_anywhere() {
  local meta line
  meta="$TMP_ROOT/direct-pr-gate.meta"
  mkdir -p "$TMP_ROOT"
  printf 'kind=ship\nmode=direct-PR\n' > "$meta"
  for line in \
    "done: PR https://github.com/o/r/pull/12" \
    "done: raised https://gitlab.com/g/s/p/-/merge_requests/9." \
    "done: shipped (https://github.com/o/r/pull/12)" \
    "done: PR <https://github.com/o/r/pull/12>" \
    "done: shipped [PR](https://github.com/o/r/pull/12)" \
    "done: PR https://github.com/o/r/pull/12/files" \
    "done: PR https://github.com/o/r/pull/12#issuecomment-99" \
    "done: PR https://review.example.org/c/team/app/+/42 published for review"; do
    fm_dod_accept_direct_pr_done ship direct-PR "$line" "$meta" >/dev/null \
      || fail "a direct-PR done naming its pull request must pass: $line"
  done
  for line in \
    "done: implemented and committed on the branch" \
    "done: read https://example.invalid/docs/pull-requests first" \
    "done: PR #12 checks green"; do
    fm_dod_accept_direct_pr_done ship direct-PR "$line" "$meta" >/dev/null \
      && fail "a direct-PR done naming no pull request must be refused: $line"
  done
  fm_dod_accept_direct_pr_done ship no-mistakes "done: implemented" "$meta" \
    || fail "a no-mistakes pre-validation done is the pipeline handoff and must pass"
  fm_dod_accept_direct_pr_done ship local-only "done: ready in branch fm/x" "$meta" \
    || fail "local-only delivery finishes without a PR and must pass"
  fm_dod_accept_direct_pr_done scout direct-PR "done: findings written" "$meta" \
    || fail "a scout keeps its own report gate and must pass"
  printf 'kind=ship\nmode=direct-PR\npr=https://github.com/o/r/pull/12\n' > "$meta"
  fm_dod_accept_direct_pr_done ship direct-PR "done: PR ready, awaiting captain merge" "$meta" \
    || fail "a recorded pull request must satisfy the gate however the done is worded"
  pass "the direct-PR done gate finds a pull request in the record, the line, or its prose"
}

test_no_run_idle_pane_direct_pr_done_without_pr_needs_the_delivery_steer() {
  reset_fakes
  local d; d=$(make_direct_pr_done_case done-no-pr-direct feat-nopr-d direct-PR)
  printf 'done: implemented and committed on the branch\n' > "$d/state/feat-nopr-d.status"
  local out; out=$(run_crew_state "$d" feat-nopr-d)
  assert_not_contains "$out" "state: done" "a done with no pull request is not a finished delivery"
  ...
  local logged; logged=$(cat "$d/state/feat-nopr-d.status")
  assert_contains "$logged" "done: implemented and committed on the branch" \
...
test_direct_pr_done_refused_by_the_named_head_keeps_that_reason() {
  reset_fakes
  local d; d=$(make_direct_pr_done_case done-unpushed-direct feat-unp-d direct-PR)
  git -C "$d/wt" commit -q --allow-empty -m unpushed
  printf 'done: implemented and committed on the branch\n' > "$d/state/feat-unp-d.status"
  local out; out=$(run_crew_state "$d" feat-unp-d)
  assert_contains "$out" "state: blocked" "an unpushed direct-PR done is blocked"
  assert_contains "$out" "unreachable outside the worker copy" "the named-head refusal keeps its own reason"
  assert_not_contains "$out" "steer it to push" "the presence gate runs only after the named head is accepted"
  pass "a direct-PR done refused by the named-head gate keeps that reason"
}

test_no_run_idle_pane_done_with_pr_still_done() {
  reset_fakes
  local d; d=$(make_direct_pr_done_case done-with-pr feat-pr direct-PR)
  printf 'done: PR https://github.com/o/r/pull/12\n' > "$d/state/feat-pr.status"
  ...
  pass "direct-PR ship: a done naming its PR is untouched"
}

test_no_run_idle_pane_done_with_recorded_pr_still_done() {
  reset_fakes
  local d; d=$(make_direct_pr_done_case done-recorded-pr feat-recpr direct-PR \
    "pr=https://github.com/o/r/pull/12")
...
test_no_run_idle_pane_done_with_an_earlier_pr_announcement_still_done() {
  reset_fakes
  local d; d=$(make_direct_pr_done_case done-announced-pr feat-annpr direct-PR)
  {
...
test_no_run_idle_pane_local_only_and_scout_done_untouched() {
  reset_fakes
  local d out
  d=$(make_direct_pr_done_case done-no-pr-modes feat-lo local-only)
  ...
  fm_write_meta "$d/state/scout-x.meta" \
    "window=fm:fm-scout-x" "worktree=$d/wt" "kind=scout" "mode=direct-PR" "harness=claude"
  arm_idle_record "$d/state" scout-x
