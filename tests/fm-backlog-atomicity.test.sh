...
if [ "\${1:-}" = return ] && [ ! -f "$case_dir/teardown-interrupted" ]; then
  ...
  # Walk up past teardown's own subshells (the command substitution and the
  # grouping around this call keep teardown's command line) to teardown itself.
  teardown_pid=\$PPID
  while :; do
    parent=\$(ps -o ppid= -p "\$teardown_pid" | tr -d ' ')
    case "\$parent" in ''|*[!0-9]*) exit 1 ;; esac
    case "\$(ps -o args= -p "\$parent")" in
      *fm-teardown.sh*) teardown_pid=\$parent ;;
      *) break ;;
    esac
  done
