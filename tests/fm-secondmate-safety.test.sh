...
# next task that reuses that id, which is why the child sweep removes state/<id>.*
# as a glob rather than an enumerated list. The fixture covers records no child
# removal list names, a directory record, and one no writer names at all; the
...
endpoint_task_id=cm1
worktree=$subhome/no-such-worktree
project=$subhome/no-such-project
harness=claude
backend=herdr
herdr_session=firstmate
herdr_workspace_id=wC
herdr_tab_id=wC:tC
herdr_pane_id=fm-cm1
kind=ship
mode=no-mistakes
yolo=off
EOF
  for suffix in .status .turn-ended .pi-ext.ts .control-relaunch .control-relaunch.note \
    .gemini-settings.json .herdr-presentation .not-in-any-removal-list; do
    printf 'child record\n' > "$subhome/state/cm1$suffix"
  done
  mkdir -p "$subhome/state/cm1.inbox"
  printf 'steer\n' > "$subhome/state/cm1.inbox/m1"
  ...
  PATH="$fakebin:$PATH" FM_ROOT_OVERRIDE="$fmroot" FM_HOME="$home" FM_FAKE_TMUX_LOG="$log" \
    ...
    "$ROOT/bin/fm-teardown.sh" domain --force >/dev/null 2>"$TMP_ROOT/slot-records.err" \
    || fail "force teardown failed to retire a secondmate occupying a treehouse slot: $(cat "$TMP_ROOT/slot-records.err")"
...
EOF
  printf '%s\n' '- domain - design domain (home: '"$subhome"'; scope: design domain; projects: alpha; added 2026-06-22)' > "$home/data/secondmates.md"
  printf 'journal\n' > "$subhome/state/cm1.herdr-presentation"
  printf 'relaunch note\n' > "$subhome/state/cm1.control-relaunch.note"
  ...
  [ ! -e "$subhome/state/cm1.control-relaunch.note" ] \
    || fail "a child relaunch note survived an ordinary retirement into the returned home"
...
# window, after the child sweep has already run - names work the captain never
# authorized this retirement to discard, so giving up the home must refuse rather
# than strand that child. The leftover journal was swept while cm1 had no meta.
...
EOF
  ...
  grep -qF 'still has a meta' "$err" \
    || fail "the late child meta did not stop the retirement; a different gate did"
  ...
  [ -e "$home/state/domain.meta" ] || fail "ordinary retirement cleared the parent meta after refusing"
  grep -F -- '- domain ' "$home/data/secondmates.md" >/dev/null \
    || fail "ordinary retirement dropped the registry route after refusing"
