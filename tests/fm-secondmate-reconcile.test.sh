...
# tests/fm-send-remote-delivery.test.sh. That leg rings the remote mate's Herdr
# endpoint after the durable inbox write, so a fake herdr rides along: its server
# is down and cannot be started, and its pane is gone, which is the ring's
# best-effort failure path, and nothing it answers starts a real server. Its calls
# are logged to <dir>/herdr.log.
