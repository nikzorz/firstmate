...
test_guard_warnings() {
  ...
  # Pin grok, a harness where the model itself arms, so the banner carries the
  # repair command these assertions order after the drain; the host test
  # runner's harness ancestry cannot change this fixture.
  printf 'project=x\n' > "$state/task.meta"
  printf 'project=y\n' > "$state/task2.meta"
