...
SH
  ...
  # fm-adopted-process-lib.sh: teardown sources it for the ownership scan that
  # keeps cleanup away from a service it did not start.
  ln -s "$ROOT/bin/fm-adopted-process-lib.sh" "$fake/bin/fm-adopted-process-lib.sh"
...
SH
  ...
  ln -s "$ROOT/bin/fm-nm-run-lib.sh" "$fake/bin/fm-nm-run-lib.sh"
  ln -s "$ROOT/bin/fm-lock-lib.sh" "$fake/bin/fm-lock-lib.sh"
  # See make_fake_root: the same two libraries teardown sources.
  ln -s "$ROOT/bin/fm-adopted-process-lib.sh" "$fake/bin/fm-adopted-process-lib.sh"
  ln -s "$ROOT/bin/fm-episode-records-lib.sh" "$fake/bin/fm-episode-records-lib.sh"
  # fm-lease-lib.sh: teardown sources it for the supervision lease guard.
  ln -s "$ROOT/bin/fm-lease-lib.sh" "$fake/bin/fm-lease-lib.sh"
