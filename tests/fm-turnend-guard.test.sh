...
# A Claude-only entry with no Grok counterpart, such as the context reset
# reminder that reads Claude's own transcript, refuses Grok the same way.
#
...
test_tracked_claude_entries_inert_under_grok() {
  ...
  for script in fm-turnend-guard.sh fm-claude-stop-autoarm.sh fm-sessionstart-run.sh \
    fm-arm-pretool-check.sh fm-cd-pretool-check.sh fm-subagent-pretool-check.sh fm-context-reset-reminder.sh; do
