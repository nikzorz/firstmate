...
#
# And teardown-kills-shared-validation-daemon: `treehouse return` and teardown's own
# leaked-process reap both end every process whose working directory is inside the
# worktree, so a shared service that happens to sit there would die with the task's
# own agent. Measured once as the shared validation daemon plus two other lanes'
# in-flight runs.
...
#   (ta) no lock, worktree clean and landed                   -> retry ALLOW
#   (tb) no lock, return target ref moved across the attempt  -> movement recorded
#   (tc) no lock, a straggler left uncommitted work           -> abort, no retry
#   (td) no lock, a straggler left an unlanded commit         -> abort, no retry
#   (te) no lock, --force over work that lives nowhere else   -> abort, no retry
#   (tf) no lock, failure never clears                        -> abort after the window
#   (tg) no lock, no remote, HEAD in the local default branch -> retry ALLOW
#   (th) no lock, no remote, HEAD absent from that branch     -> abort, no retry
#   (ti) no lock, no remote and no default branch at all      -> abort, no retry
#   (tj) no lock, worktree unopenable after the failure       -> abort, named as uninspectable
#   (tk) no lock, the FINAL attempt leaves work behind         -> abort, gap reported not asserted
set -u
...
# A case with no remote at all, built on make_case so it keeps the hermetic
# no-mistakes stub and the data/ directory teardown closes the backlog in: local
# -only work lands only by reaching the project's own default branch.
make_local_case() {
  local name=$1 case_dir
  case_dir=$(make_case "$name")
  rm -rf "$case_dir/wt" "$case_dir/project" "$case_dir/origin.git"
...
# The adopted-process fixtures below keep a pid registry in a file whose path is
# fixed here rather than on first use: the spawn helpers run inside command
# substitutions, so anything they assign to a shell variable is assigned in a
# subshell and never reaches this one.
mkdir -p "$TMP_ROOT"
...
if [ "\${1:-}" = kill-window ] && [ "\${2:-}" = -t ]; then
  # The tmux adapter names a window exactly, as =session:=window.
  target=\${3:-}
  target=\${target//=/}
...
# records on the endpoint (the target folded to a watcher key), outside any
# state/<id>.<suffix> record. Teardown reclaims them with the task (the inheritance
# guarantee itself lives at the spawn claim), and must leave a live neighbor whose
# key merely extends this one alone.
test_teardown_reclaims_the_endpoint_keyed_supervisor_records() {
  local case_dir s key f
  ...
  printf '1\n' > "$s/.subsuper-pause-until-due-task-x1"
  printf '1\n' > "$s/.seen-task-x1_status"
  printf '2\n' > "$s/.wedge-escalations-${key}1"
  printf '1\n' > "$s/.subsuper-stale-task-x11"

  run_teardown "$case_dir" > "$case_dir/stdout" 2> "$case_dir/stderr" \
    || fail "endpoint-keyed-records: teardown failed: $(cat "$case_dir/stderr")"

  for f in ".wedge-escalations-$key" ".stale-$key" ".paused-$key" ".writing-since-$key" \
    ".dead-reported-$key" ".hash-$key" .subsuper-stale-task-x1 \
    .subsuper-pause-until-due-task-x1 .seen-task-x1_status; do
    assert_absent "$s/$f" "endpoint-keyed-records: $f outlived its task"
  done
  assert_present "$s/.wedge-escalations-${key}1" \
    "endpoint-keyed-records: a live neighbor's escalation count was cleared as collateral"
  assert_present "$s/.subsuper-stale-task-x11" \
...
test_nested_stop_names_what_an_outer_frame_already_destroyed() {
  ...
  fm_write_secondmate_meta "$home/state/domain.meta" "$subhome"
  # No registry: a nested secondmate home can carry no binding in it, so any
  # registry here would refuse the nested removal at prevalidation, before the
  # recursion whose stop report this case is about.
