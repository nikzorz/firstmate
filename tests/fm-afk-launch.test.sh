  ...
  keep_awake_cycle() {  # <home> <stub>
    ...
    FM_HOME="$1" FM_STATE_OVERRIDE="$1/state" FM_CONFIG_OVERRIDE="$1/config" \
      STUB="$2" bash -c '
        ...
        printf "stop rc=%s\n" "$?"
      ' _ "$LAUNCH" 2>&1 \
      | sed -e "s|$1|<home>|g" -e 's|afk-contracts/[0-9]*\.afk-contract|afk-contracts/<epoch>.afk-contract|g'
  }
  # The stand-down names the archived record by its home and entry second, which
  # differ between the two runs, so both are normalized before comparing.
  out_base=$(keep_awake_cycle "$base" 1)
  out_live=$(keep_awake_cycle "$live" 0)
  # Both runs must actually arm and stand down, or the comparison proves nothing.
  case "$out_live" in
    *"enter failed"*) fail "keep-awake absent: the fixture could not enter away mode" ;;
    *"start-native rc=0"*"stop rc=0"*) ;;
    *) fail "keep-awake absent: the fixture lifecycle did not arm and stand down: $out_live" ;;
  esac
...
# ---------------------------------------------------------------------------
# The launcher drives keep-awake: every daemon-backed entry arms it, away and
# quiet alike, while stand-down and the crash reconcile release it. A posture
# with no away-mode daemon (Pi) never arms it at all.
# ---------------------------------------------------------------------------
unit_keep_awake_follows_the_launch_lifecycle() {
  local home pwsh holder mode
  # Runs the launcher against <home> with the fake Windows shell armed.
  keep_awake_launch() {  # <home> <pwsh> <launcher args...>
    local home=$1 pwsh=$2
    shift 2
    FM_HOME="$home" FM_STATE_OVERRIDE="$home/state" FM_CONFIG_OVERRIDE="$home/config" \
      FM_KEEP_AWAKE_INTEROP="$home/config/keep-awake" FM_KEEP_AWAKE_PWSH="$pwsh" \
      "$@" >/dev/null 2>&1
  }
  for mode in away quiet; do
    ...
    keep_awake_launch "$home" "$pwsh" "$LAUNCH" enter --words 'keep-awake fixture'
    keep_awake_launch "$home" "$pwsh" env FM_AFK_MODE="$mode" "$LAUNCH" start-native
    holder=$(cut -f1 "$home/state/.afk-keep-awake" 2>/dev/null || true)
    if [ -e "$home/state/.afk" ] && [ -n "$holder" ] && kill -0 "$holder" 2>/dev/null; then
      pass "keep-awake launch ($mode): a daemon entry arms the request"
    else
      fail "keep-awake launch ($mode): a daemon entry left no live holder"
    fi
    keep_awake_launch "$home" "$pwsh" "$LAUNCH" stop
    if keep_awake_wait_gone "$holder" 10 && [ ! -e "$home/state/.afk-keep-awake" ]; then
      pass "keep-awake launch ($mode): stand-down releases the request"
    else
      fail "keep-awake launch ($mode): the request outlived stand-down"
    fi
  done
  ...
  pwsh=$(keep_awake_fake_pwsh "$home")
  keep_awake_launch "$home" "$pwsh" "$LAUNCH" enter --words 'keep-awake fixture'
  keep_awake_launch "$home" "$pwsh" "$LAUNCH" start-native
  holder=$(cut -f1 "$home/state/.afk-keep-awake" 2>/dev/null || true)
  keep_awake_launch "$home" "$pwsh" "$LAUNCH" reconcile
  if [ -n "$holder" ] && keep_awake_wait_gone "$holder" 10 \
    && [ ! -e "$home/state/.afk-keep-awake" ]; then
    pass "keep-awake launch: the crash reconcile releases a request no live daemon owns"
  else
    fail "keep-awake launch: the crash reconcile left the request held"
  fi
  ...
  pwsh=$(keep_awake_fake_pwsh "$home")
  keep_awake_launch "$home" "$pwsh" "$LAUNCH" enter --words 'keep-awake fixture'
  # shellcheck disable=SC2016 # The sourced launcher expands these, not this shell.
  keep_awake_launch "$home" "$pwsh" env FM_TEST_HARNESS=pi bash -c \
    '. "$1"; fm_afk_launch_primary_harness() { printf "%s" "$FM_TEST_HARNESS"; }; fm_afk_launch_main start-native' \
    _ "$LAUNCH"
  if [ ! -e "$home/state/.afk-keep-awake" ]; then
    pass "keep-awake launch: a daemon-less Pi posture never arms the request"
  else
    fail "keep-awake launch: a daemon-less Pi posture armed the request"
  fi
  unset -f keep_awake_launch
}
