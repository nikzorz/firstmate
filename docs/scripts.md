...
| `fm-decision-hold.sh`    | Create, verify, complete, and resolve durable captain-held decisions; link and reconcile captain-gated items against worker gates |
...
| `fm-session-lock-lib.sh` | Shared session-lock harness identity (ancestry walk, holder liveness, the same-session-under-a-new-pid rule, and the ownership classification that keeps one home to one owner) for fm-lock.sh and the Claude Stop auto-arm |
...
| `fm-supervision-instructions.sh` | Render the session-start primary-harness supervision block, the one-line repair instruction, or the stale-beacon advice line |
...
| `fm-project-mode.sh`     | Resolve a project's delivery mode, `+yolo` flag, and project-memory posture from `data/projects.md`           |
...
| `fm-keep-awake.sh`       | Hold and release the opt-in Windows system-awake request for the armed away-mode window |
...
| `fm-tasks-axi-lib.sh`    | Shared backlog-backend selector, `tasks-axi` compatibility probe, and the guarded single-item backlog read that separates a genuine absence from a store it could not establish |
...
| `fm-send-result-lib.sh`  | Shared reader of `fm-send.sh`'s exit status for every caller that acts on the outcome |
...
| `fm-pr-merge.sh`         | Record PR metadata, then merge a task's canonical full GitHub URL, reporting any issue the task's record links that the body does not close by keyword and any that is still open afterwards |
