...
| [`fm-dod-lib.sh`](../bin/fm-dod-lib.sh) | Own ship/scout worker role scope, ship definitions of done, the named-head reachability gate on ship `done:` acceptance, the direct-PR pull-request presence gate that `fm-crew-state.sh` composes after it, and the no-mistakes `--intent` contract |
