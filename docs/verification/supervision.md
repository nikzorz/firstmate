...
This record supports current session-start, turn-end, watcher-continuity, supervision-host, wedge-alarm, away-mode keep-awake, usage-limit, and run-inactivity guarantees.
...
The opt-in gate, fail-open behavior, idempotence, release on stand-down, and release after the away-mode daemon dies are covered without any real power state by `tests/fm-afk-launch.test.sh`, which runs the whole lifecycle against a fake Windows shell, including that both away and quiet postures arm the holder and a Pi primary never does.
...
The read is `quota-axi --provider claude --json --no-credential-refresh`, admitted only through `fm_quota_json_valid` in `bin/fm-quota-axi-lib.sh` and bound to the default claude account row through that library's `quota_row` join, so a snapshot the fleet's own validator refuses reads `unknown` here too.
Measured on 2026-09-24 with the installed quota-axi 0.1.34 against a Claude account, alongside Claude Code 2.1.281.

```sh
quota-axi --provider claude --json --no-credential-refresh > snapshot.json
jq '{schemaVersion,
     ...
     semantics: .providers[0].quotaSemantics
       | {status, all_models: (.effectiveAvailability[]
                               ...
                               | {status, effectivePercentRemaining, boundedBy, limitingWindowIds})}}' snapshot.json
bash -c '. bin/fm-quota-axi-lib.sh; fm_quota_json_valid < snapshot.json && echo VALID || echo INVALID'
```
...
```json
{
  ...
  "windows": [
    { "id": "five_hour",   "percentRemaining": 92,  "resetsAt": "2026-09-24T20:29:59.733310+00:00" },
    { "id": "seven_day",   "percentRemaining": 81,  "resetsAt": "2026-09-29T05:59:59.733333+00:00" },
    { "id": "model:fable", "percentRemaining": 100, "resetsAt": "2026-09-29T06:00:00+00:00" }
  ],
  "semantics": {
    "status": "known",
    "all_models": {
      "status": "known",
      "effectivePercentRemaining": 81,
...
The reset time adds four more fields to that dependency, all present above: `providers[].windows[].id`, `.percentRemaining`, `.resetsAt`, and `effectiveAvailability[].boundedBy`.
...
Note that `boundedBy` is broader than `limitingWindowIds`: the recorded output bounds on both `five_hour` and `seven_day` while only `seven_day` is currently limiting.
...
`resetsAt` is a UTC ISO 8601 timestamp with a numeric `+00:00` offset, with microseconds on the account windows and none on `model:fable`, and `fm_claude_limit_parse_iso8601` accepts both shapes.
`model:fable` is the useful counter-example in this capture: it is not listed in `boundedBy`, so it never enters the set the reset time is taken from, whatever its own figures say.
...
A future reset, plus `FM_CLAUDE_LIMIT_RESET_GRACE_SECS`, is written into the recorded `paused:` line as ` until <YYYY-MM-DDTHH:MM:SSZ>`, which `status_paused_until` in `bin/fm-classify-lib.sh` reads.

Every unreadable, unparseable, schema-invalid, provider-stale, or not-`known` input reports `unknown` with no reset time, which authorizes neither recovery nor a settled wait; `tests/fm-limit-resume.test.sh` covers each of those inputs without touching a real account.
A readable window whose reset time is absent or unparseable is still a good `exhausted` verdict - only the `until` time is lost, and the supervisors fall back to `FM_PAUSE_RESURFACE_SECS`.
If the provider ever renames any of those fields, that fallback is what happens silently, so this record is the thing to re-run when a quota-axi upgrade lands.
...
The v1.70.1 capture in `tests/captures/no-mistakes-v1.70.1/replacement.toon` (captured 2026-09-14, provenance in that directory's README) carries the same column names plus a `round_active_for` column the name lookup ignores:
...
That ci cell is quiet past the default remote budget while its own last activity is the green monitoring marker, which is why such a step is exempt from the budget while its ci log gives no marker of its own, and a readable log still decides.
...
The reader requests the whole log with `--full` rather than this tail, and the stored file above is that whole log, so no line of it carries a time either.
