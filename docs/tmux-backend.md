...
A typed `fm-send.sh` submit scopes both conversions' busy reads to the target's recorded harness, so a harness whose spinner is outside the generic signature union (Claude's elapsed-time spinner) still confirms a delivered message, while a recorded harness with no verified signature of its own falls back to that union for this check only.
