...
That message is the forge's own default, read back with plain `gh` because `gh-axi` wraps its response in a display envelope, so `gh` must be on `PATH` for a squash merge the caller wrote no message for.
