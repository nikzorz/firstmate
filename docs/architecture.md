...
Actionable wakes include captain-relevant status signals, no-verb signals whose crew is not provably working, authenticated check output such as PR merge polling or an X-mode mention, stale panes whose crew is neither provably working, nor finished with an armed merge poll to wake on, nor parked on a still-open decision only firstmate or the captain can answer, whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS` without a still-advancing run step behind a confirmed-live endpoint, stale panes an advancing run step has kept absorbed past `FM_PAUSE_RESURFACE_SECS`, declared external waits that remain paused past `FM_PAUSE_RESURFACE_SECS` or reach a recorded end time of their own, and heartbeat backstop hits.
A crew that declared an external wait is graded against its own endpoint before any of that applies: a confirmed-live endpoint with a no-mistakes run step attributed to its branch that is both non-terminal and still advancing takes the long paused cadence, while the same run behind a dead or unreadable endpoint keeps the wedge timer, because a no-mistakes run proves nothing about the crew that started it.
A run that has stopped advancing reads `stalled` and no longer earns that cadence, and a non-terminal status alone still earns it only where no such evidence is available, which [`configuration.md`](configuration.md#declared-pause-absorb-by-verdict-and-endpoint-liveness) enumerates alongside the outcome for every verdict and endpoint reading.
The long cadence still rechecks the crew state every `FM_STALE_ESCALATE_SECS`, memoizing the endpoint reading that earned it in between so a repeat poll costs one cheap liveness probe instead of another crew-state read.
Only that out-of-band run evidence lets a crew that still reads as working earn the long cadence - a crew whose sole corroboration is its own busy pane signature keeps the wedge timer, since that signature is read from the very pane the stale wake already found unchanged, and a failed run step is treated as evidence of nothing rather than as proof the crew stopped.
...
The long-cadence rechecks the still-advancing-run absorb raises are capped by that same count within one stale episode, so a crew that is alive but whose advancement cannot be measured converts to a demand-deep-inspection escalation rather than re-surfacing on that cadence forever.
...
No-verb wakes, such as `working:` notes and bare turn-ended signals, are benign only when `bin/fm-crew-state.sh` reports positive evidence that the crew is still working: an actively running no-mistakes step it attributes under the run-attribution rule below, or a backend busy signature.
A crew that declares `paused:` for a known external wait is separately absorbed while idle and re-surfaced on the longer pause cadence - or earlier, when that wait recorded an end time of its own - rather than being treated as a possible wedge.
...
Two gates gate that absorb, and a crew that fails either one still surfaces: the work must actually have somewhere to land, since a finished crew with no recorded pull request and no armed merge poll has stopped with nowhere to go, and the crew's own last status line must itself be a terminal done, since a `needs-decision:` or `blocked:` appended after the run finished is a crew asking for help rather than one waiting on a merge.
...
Four gates gate that absorb as well.
The park must come from an attributed no-mistakes run whose gate ownership token says the answer belongs to firstmate or the captain, which `bin/fm-crew-state.sh` computes from the `action` column of the gate's findings table and from the gate's status word being readable at all, rather than from any text appearing elsewhere in the run output.
Which readable word it is decides nothing, because an ask-user finding sitting at a fix-review gate is one the worker may not answer just as much as one sitting at an approval gate, and the fix-review shape is the one most real escalation parks take.
A park at a gate whose findings carry no ask-user row, and the run-less fallback that reads `parked` straight off the crew's own `needs-decision:` line, both stay outside that limit deliberately and keep surfacing, because neither is evidence that anyone else owes this crew an answer and absorbing on them could silence a wedged worker indefinitely.
A park whose gate status word neither of that script's two probes can read is excluded for the same reason: an unread owner is not an owner, so no token is published and the crew keeps surfacing exactly as it did before this absorb existed.
That exclusion can only produce wakes and can never silence one, and [`verification/supervision.md`](verification/supervision.md#gate-status-word-on-a-parked-run) records what real parked runs published when the rule was measured.
A park that carries an ask-user finding without earning the token - one whose status word could not be read - still carries an operator-facing note reporting that finding, deliberately spelled so it cannot be mistaken for the ownership token, and naming no owner because firstmate reads this line to decide whether to escalate.
The run must also not have re-parked since the crew last spoke, which firstmate decides by comparing two sightings of the same task rather than by reading any clock.
`bin/fm-crew-state.sh` publishes an identity for the park episode, composed from the gate name, the run head and the gate's findings table, and `bin/fm-classify-lib.sh` keeps the last identity it saw for a task alongside a fingerprint of everything the crew had said at that sighting, in `state/<id>.park-sighting`.
A later sighting that carries a different park identity while the crew's record is unchanged is proof the run re-parked and the crew has said nothing since, which is exactly the shape that otherwise passes every other gate: firstmate answers, the worker responds to the gate, the pipeline fixes, the run parks again, and a worker that wedges before saying anything about the new gate leaves its old decision standing over a park it never announced.
That sighting is not overwritten while the crew stays silent, so the verdict holds on every later look rather than adopting the new park as a fresh baseline.
That the identity's material actually moves from one park episode to the next is a premise about no-mistakes rather than about firstmate, so [`verification/supervision.md`](verification/supervision.md#park-identity-movement-between-consecutive-park-episodes) records what real consecutive park episodes published, over how few of them, and what it would take to break it.
Both supervision modes record sightings, including away mode, where the daemon owns triage and the watcher surfaces a stale crew without classifying it at all: recording there is what keeps the record from carrying a hole as wide as the away window.
A task with no sighting recorded yet and an unreadable status file both withhold the absorb, so every park this gate cannot decide about surfaces exactly as it did before the gate existed.
The identity itself always has material to fingerprint: it is composed only for a park whose gate findings carry an ask-user row, and that row is read out of the very findings table the identity is built from.
...
Those four gates are separate facts that agree, not one fact implying another: the ownership token says who owns the gate, the two sightings say the run has not re-parked since the crew last spoke, and the status stream says a decision is open, but none of them ties the open key to a particular finding, because the stream is keyed per task while a park names no key.
Three costs that leaves are stated rather than hidden.
A task firstmate has never yet seen parked has nothing to compare against, so its first sighting escalates and only the sighting after it can absorb; a crew already wedged in the re-parked-and-silent shape at the moment its first sighting is recorded is therefore absorbed from the next look, which is a permanent absorb rather than a single notification.
What keeps that rare is taking the sighting while the park behind the crew's word is still the one it was written at: the wake that surfaces a `needs-decision:` records one, as do both stale paths, so the ordinary announce-then-wait flow never reaches that position.
A crash, a restart, a cleared state directory, and a decision announced while its run was not yet parked or could not be read are what still do.
The second is the gap between two sightings, which are taken on the wake that announces a decision and once per stale episode, rather than continuously.
A word the crew appends and a re-park that both fall inside one gap cannot be placed on either side of each other, so firstmate answers from the record as it stood, which escalates that look, and then adopts the new park as the baseline.
If the word was not about the new park, every look after that absorbs, so the residual is a permanent absorb rather than an imprecise date.
Narrowing the gap is what keeps it rare, which is why every path that sees a stale crew records a sighting even when it does not classify one; refusing to write when both moved would make the verdict stick, but it would then escalate forever for the crew that announces each park it reaches, which is the shape this absorb exists to serve.
The third is the price of failing toward surfacing: a park whose gate status word cannot be read earns no ownership token at all, and a task whose status file cannot be read is never weighed, so both keep the whole notification volume this absorb exists to remove.
That is the deliberate trade rather than an oversight, because a park absorbed on evidence nobody has leaves no timer and no other wake owner behind it.
...
The submitted head is what carries code identity there, and the binding outlives the run, so a crew that has committed further work since a finished run is no longer attributed to it.
...
A still-exhausted window is recorded with the existing `paused:` vocabulary, so declared-pause handling owns the recheck; recovery once the window has reset is the guarded `bin/fm-limit-resume.sh`, which re-proves the live match before it sends anything.
That pause also carries when it ends, because the fixed recheck cadence alone made recovery correct but up to an hour late: the same quota read that proves the window is exhausted reports when it resets, so the recovery script records one recheck epoch in `state/<id>.pause-recheck` and both supervisors recheck at whichever comes first, that epoch or `FM_PAUSE_RESURFACE_SECS`.
Three properties keep it a scheduling refinement rather than a second cadence, and `bin/fm-classify-lib.sh` owns all three: it is OR-ed with the fixed window so it can only ever fire earlier, it is one-shot and cleared as it fires so an elapsed epoch cannot re-fire on every poll, and only a future epoch is ever recorded so a recheck that finds the window still exhausted returns to the fixed cadence instead of rescheduling itself into a spin.
An absent, unparseable, or unreadable reset time simply records no epoch, which leaves the fixed cadence in charge exactly as before.
`bin/fm-claude-limit-lib.sh` parses that timestamp arithmetically rather than through `date`, whose ISO 8601 handling differs between GNU and BSD, so the schedule cannot depend on which platform the home runs on.
Because the crew never learns that firstmate wrote that pause, the recover path also closes it once it has dismissed the prompt and submitted the steer, whether or not that submission could be confirmed, and only ever the line it opened itself, so a resume that silently does not take ages on the wedge cadence rather than the hour-long pause recheck.
...
When only an owned child's current classification is unavailable, the home keeps the classification its readable children positively support, and independently trustworthy structured decisions, holds, queued and landed records, endpoint identities, counts, and provenance remain available.
It falls back to unknown there only for the negative no-active-work answer, which is a claim about every child that a home holding an unreadable one cannot make, and it loses strict validity either way so the read is marked partial rather than complete.
...
On `attached` it stays live across identity-matched successors, and a clean child close either attaches to a verified healthy successor, reports the delivered wake the durable queue's counter proves, or becomes the typed nonzero `watcher: FAILED - cycle ended without an actionable reason` result.
...
That banner's closing advice comes from `bin/fm-supervision-instructions.sh`, so a stale beacon never recommends a repair the detected harness's own supervision protocol forbids.
...
That bound asks the same question the watcher's wedge timer asks, through the same `crew_run_step_advancing`: an idle pane whose run step is still advancing behind a confirmed-live endpoint is absorbed rather than reported as a possible wedge, while a run that has stopped advancing escalates unchanged.
Sharing that predicate is the boundary that matters here, because it is what stops the two supervision paths drifting on what advancing means for one crew; the still-advancing-run absorb section of `bin/fm-supervise-daemon.sh` owns the away-mode contract in full, including its cadence, its `FM_WEDGE_DEMAND_INSPECT_COUNT` cap, and what that cap does and does not promise.
The watcher's `wedge_timer_check` applies that same cap to its own absorb, so one policy governs both paths.
The two paths keep their own re-surface machinery rather than sharing it, since the watcher exits its cycle through `fm_wake_append`/`wake` while the daemon buffers through `escalate_add` into a batched digest.
The daemon escalates captain-relevant events, plus a bounded recheck for a declared pause or a still-advancing run that remains idle, as one batched, single-line digest using the canonical `away-supervisor` kind from `bin/fm-operational-input.sh` so firstmate can distinguish it structurally from real messages.
...
`data/projects.md` records each project's delivery mode, optional `+yolo` autonomy flag, and optional `+keep-claude-md` project-memory marker.
...
On the squash path it also supplies the commit subject and body rather than letting the forge compose them, so the agent attribution trailers `bin/fm-attribution-lib.sh` recognises never reach the default branch.
Ownership of that message splits by half: a caller's own `--body` or `--body-file` takes the body and the strip stands down, while a caller's `--subject` takes only the subject and the stripped body is still supplied underneath it, because the trailers live in the body.
That message is the forge's own default, read back with plain `gh` because `gh-axi` wraps its response in a display envelope, so `gh` must be on `PATH` for a squash merge the caller wrote no body for.
A default message the forge will not return stops the merge, because falling back to a forge-composed one is the case this exists to prevent; [`docs/verification/agent-attribution.md`](verification/agent-attribution.md) holds the evidence and the limits.
...
The merge is also the last point at which nothing can rewrite the PR body, so the helper reports there on any issue the task's own backlog record links that the body does not close with a well-formed keyword, and afterwards reads back the state of the ones it does close, because no keyword form can fool a state read.
Alone among this helper's guards that one only reports and never refuses: a linked issue the work merely mentions for context is indistinguishable from one it owns, so a refusal would hand that same guess to whoever had to satisfy it.
[`bin/fm-pr-merge.sh`](../bin/fm-pr-merge.sh)'s header owns the accepted keyword and reference forms, the three answers the backlog read has, the paths where either read is skipped, and the wording each outcome gets.
...
A project the registry marks `+keep-claude-md` instead keeps `CLAUDE.md` as its real project-memory file, gets no `AGENTS.md`, and carries that prohibition in its generated ship brief.
