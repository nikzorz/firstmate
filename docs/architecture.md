...
Actionable wakes include captain-relevant status signals, no-verb signals whose crew is not provably working, authenticated check output such as PR merge polling or an X-mode mention, stale panes whose crew is neither provably working nor finished with an armed merge poll to wake on, whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS`, declared external waits that remain paused past `FM_PAUSE_RESURFACE_SECS` or reach a recorded end time of their own, and heartbeat backstop hits.
...
That exclusion can only produce wakes and can never silence one, and `docs/verification/supervision.md` records what real parked runs published when the rule was measured.
