...
A pull request that is already merged skips that read entirely, so re-running a merge that already landed stays clean without forge access, while a merged state that cannot be read falls through to the ordinary read and its refusal rather than being taken for either answer.
