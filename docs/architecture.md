...
Actionable wakes include captain-relevant status signals, no-verb signals whose crew is not provably working, authenticated check output such as PR merge polling or an X-mode mention, stale panes whose crew is neither provably working, nor finished with an armed merge poll to wake on, nor parked on a still-open decision only firstmate or the captain can answer, whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS` without a still-advancing run step behind a confirmed-live endpoint, declared external waits that remain paused past `FM_PAUSE_RESURFACE_SECS` or reach a recorded end time of their own, and heartbeat backstop hits.
