...
A park whose identity could not be composed, a task with no sighting recorded yet, and an unreadable status file all withhold the absorb, so every park this gate cannot decide about surfaces exactly as it did before the gate existed.
...
The third is the price of failing toward surfacing: a park whose identity cannot be composed earns no token at all, so it is never absorbed and keeps the whole notification volume this absorb exists to remove.
