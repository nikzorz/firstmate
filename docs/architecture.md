...
A crew that declared an external wait is graded against its own endpoint before any of that applies: a confirmed-live endpoint whose attributed no-mistakes run step is still advancing takes the long paused cadence, while the same progressing run behind a dead or unreadable endpoint keeps the wedge timer, because a no-mistakes run proves nothing about the crew that started it.
