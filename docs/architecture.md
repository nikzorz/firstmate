...
It also applies the same `FM_WEDGE_DEMAND_INSPECT_COUNT` cap, so a pane absorbed that many rechecks in a row escalates as a wedge demanding deep inspection rather than staying absorbed on an unmeasurable run step.
