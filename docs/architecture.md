...
Actionable wakes include captain-relevant status signals, no-verb signals whose crew is not provably working, authenticated check output such as PR merge polling or an X-mode mention, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS`, declared external waits that remain paused past `FM_PAUSE_RESURFACE_SECS` or reach a recorded end time of their own, and heartbeat backstop hits.
A crew that declared an external wait is graded against its own endpoint before any of that applies: a confirmed-live endpoint with a no-mistakes run step attributed to its branch reporting a non-terminal status takes the long paused cadence, while the same run behind a dead or unreadable endpoint keeps the wedge timer, because a no-mistakes run proves nothing about the crew that started it.
That attributed status now has to be advancing as well as non-terminal, because a run past its inactivity budget reads `stalled` and no longer earns the cadence; the older reading survives only where no elapsed figure exists to compare, which [`configuration.md`](configuration.md#declared-pause-absorb-by-verdict-and-endpoint-liveness) enumerates alongside the outcome for every verdict and endpoint reading.
