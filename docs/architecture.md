...
Two costs that leaves are stated rather than hidden.
...
The other is the price of failing toward surfacing: a park whose identity cannot be composed earns no token at all, so it is never absorbed and keeps the whole notification volume this absorb exists to remove.
