...
The crew's own status record must also have been written during the park episode the run is sitting in now, which `bin/fm-crew-state.sh` establishes from the park clock no-mistakes restarts each time a run stops for the agent.
...
An absent park clock, an unparseable one, an unreadable status file and a missing one all withhold the token, so every park this gate cannot decide about surfaces exactly as it did before the gate existed.
...
Those four gates are separate facts that agree, not one fact implying another: the ownership token says who owns the gate, the park clock says the crew spoke at this park, and the status stream says a decision is open, but none of them ties the open key to a particular finding, because the stream is keyed per task while a park names no key.
Two costs that leaves are stated rather than hidden.
The published park clock is truncated, and past a day of waiting it resolves to whole hours, so a decision written up to an hour before the current episode began can still read as written at it.
The other is the price of failing toward surfacing: on an installation whose no-mistakes never publishes that clock, no park can earn the token, so none is absorbed and that installation keeps the whole notification volume this absorb exists to remove.
