...
A run that has stopped advancing reads `stalled` and no longer earns that cadence, and a non-terminal status alone still earns it only where no elapsed figure exists to compare, which [`configuration.md`](configuration.md#declared-pause-absorb-by-verdict-and-endpoint-liveness) enumerates alongside the outcome for every verdict and endpoint reading.
