...
The park must come from an attributed no-mistakes run whose gate ownership token says the answer belongs to firstmate or the captain, which `bin/fm-crew-state.sh` computes from the gate's own status word and the `action` column of the gate's findings table rather than from any text appearing elsewhere in the run output.
A park at a gate the worker itself answers, including a fix review that carries an ask-user finding, and the run-less fallback that reads `parked` straight off the crew's own `needs-decision:` line, both stay outside that limit deliberately and keep surfacing, because neither is evidence that anyone else owes this crew an answer and absorbing on them could silence a wedged worker indefinitely.
A park whose gate status word neither of that script's two probes can read is the third exclusion, for the same reason: an unread owner is not an owner, so no token is published and the crew keeps surfacing exactly as it did before this absorb existed.
...
Both parked shapes that carry an ask-user finding without earning the token - a gate the worker answers, and a gate whose status word could not be read - still carry an operator-facing note reporting that finding, deliberately spelled so it cannot be mistaken for the ownership token.
That note names no owner, because those two shapes are indistinguishable once the token has been withheld and firstmate reads this line to decide whether to escalate.
...
Those three gates are separate facts that agree, not one fact implying another: the ownership token says who owns the gate, and the status stream says a decision is open, but nothing ties the open key to that particular park, because the stream is keyed per task while a park names no key.
The residual that leaves is stated rather than hidden: a task whose earlier decision was answered in pane but never closed with `resolved:`, whose run then parks at an authority-owned gate, and whose worker then wedges, stays absorbed for as long as that key stays open, and closing the key rather than the stale timer is what ends it.
