...
That attributed status records the handoff rather than verified progress - nothing compares it against a prior observation, so a hung run keeps reporting it - which is why the long cadence still rechecks the crew state every `FM_STALE_ESCALATE_SECS`, memoizing the endpoint reading that earned it in between so a repeat poll costs one cheap liveness probe instead of another crew-state read.
