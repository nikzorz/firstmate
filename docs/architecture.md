...
Nothing must show the crew's own status record to belong to an EARLIER park episode than the one the run is sitting in now, which `bin/fm-crew-state.sh` decides from the park clock no-mistakes restarts each time a run stops for the agent.
That is what ties the open decision to the park in front of it, because the shape it excludes otherwise passes every other gate: firstmate answers, the worker responds to the gate, the pipeline fixes, the run parks again, and a worker that wedges before saying anything about the new gate leaves its old decision standing over a park it never announced.
That gate withholds only on proof, never on absence: a park whose run publishes no clock, or whose clock does not parse, establishes nothing about which episode the crew's record belongs to and absorbs exactly as it did before the gate existed.
Unknown is not the same as unannounced, and the difference is load-bearing rather than pedantic, because the clock is a column on the run row and a run parked at a gate resolved from the steps table alone carries none, so reading absence as staleness would escalate every one of that run's parks forever.
...
Those four gates are separate facts that agree, not one fact implying another: the ownership token says who owns the gate, the park clock says nothing contradicts the crew having spoken at this park, and the status stream says a decision is open, but none of them ties the open key to a particular finding, because the stream is keyed per task while a park names no key.
Two residuals that leaves are stated rather than hidden.
The published park clock is truncated, and past a day of waiting it resolves to whole hours, so a decision written up to an hour before the current episode began still reads as written at it.
A park whose run publishes no clock keeps absorbing, so the silence-forever shape stays open for it, which is the deliberate trade: one notification too many is preferable to none, but escalating forever on an installation that cannot supply the evidence is worse than the noise the absorb removes.
