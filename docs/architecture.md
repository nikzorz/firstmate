...
It reads the same `FM_WEDGE_DEMAND_INSPECT_COUNT` as a per-episode cap, so a stale episode that raises that many rechecks escalates as a wedge demanding deep inspection rather than staying absorbed on an unmeasurable run step.
