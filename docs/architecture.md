...
That bound asks the same question the watcher's wedge timer asks: an idle pane whose run step is still advancing behind a confirmed-live endpoint is absorbed rather than reported as a possible wedge, on the long `FM_PAUSE_RESURFACE_SECS` recheck cadence, while a run that has stopped advancing escalates unchanged.
For a pane already inside that absorb the stalled reading arrives at the next threshold rather than on the tick that observes the stall, because the read throttle defers the busy read and the advancing read alike, and that is intended: the watcher restarts its own timer on an absorb and defers the stalled reading by one window too, so the two supervisors stay in step on one crew.
It reads the same `FM_WEDGE_DEMAND_INSPECT_COUNT` as a per-episode cap, so a stale episode that raises that many rechecks escalates as a wedge demanding deep inspection rather than staying absorbed indefinitely.
An unmeasurable run step motivates that cap but does not scope it: the advancing verdict cannot separate a measured advance from an assumed one, so the cap fires on a moving run too, trading a bounded false alarm for not missing a halted crew.
That escalation ends the episode and clears its absorb records rather than raising a permanent flag, and a next episode opens only once the pane's capture changes and settles again, so a pane frozen for the whole away window gets one demand-inspection and no repeat, as it did before this absorb existed.
