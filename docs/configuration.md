...
It reads the same `FM_WEDGE_DEMAND_INSPECT_COUNT` as a cap on one stale episode: after that many still-advancing rechecks the episode escalates as a possible wedge demanding deep inspection instead of absorbing again.
...
The captain therefore gets a bounded repeat on that sawtooth, which is the honest guarantee here, rather than a pane the daemon refuses to absorb ever again.
