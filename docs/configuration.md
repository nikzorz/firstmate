...
Away mode asks the same question on the same terms: the sub-supervisor's stale-persistence recheck (`bin/fm-supervise-daemon.sh`) absorbs a still-advancing run behind a confirmed-live endpoint rather than reporting it to the captain as a possible wedge, re-surfaces it once per `FM_PAUSE_RESURFACE_SECS` of accumulated idle age, and escalates a run that has stopped advancing exactly as before.
For a pane already absorbed, that escalation arrives at the next `FM_STALE_ESCALATE_SECS` gate opening rather than on the tick that first observes the stall, which is the intended reading: the watcher restarts its own timer on an absorb and so defers the stalled reading by one window too, and the two supervisors agreeing on one crew matters more than either jumping first.
It reads the same `FM_WEDGE_DEMAND_INSPECT_COUNT` as a cap on one stale episode: the still-advancing recheck that reaches that number escalates as a possible wedge demanding deep inspection instead of absorbing again, so one fewer than that many absorb notices arrive first, which at the default of three reads absorb, absorb, wedge.
A crew halted behind a run step nobody can measure is what motivates that cap, but the cap is not scoped to it: it fires on every capped episode, including one whose run really is moving, because the advancing verdict reports `working` for any step still inside its quiet budget and so cannot tell a measured advance from an assumed one.
That wider scope is the deliberate side of the trade, since a false alarm on a moving run is bounded to one prompt per cap while a halted crew nobody names in away mode is unbounded silence.
That escalation ends the episode rather than raising a lasting flag, because a crew that has since recovered must not stay marked as demanding inspection: the absorb records clear with the stale marker, so a crew still stuck when the next stale episode opens is absorbed afresh and escalates again a cap later.
Whether a next episode opens at all is conditional, and that is the honest limit here: in away mode one opens only once the pane's captured content changes and settles again, since an unchanged capture raises no fresh stale wake for the daemon to record a marker from.
A pane frozen for the whole away window therefore gets one demand-inspection escalation and no repeat, which is not ground lost, because that same frozen pane got one wedge line and then the same silence before this absorb existed.
Unlike the claude usage-limit question beside it, that one is not pre-filtered on the recorded harness, because a run step belongs to no harness: a stale pane on any harness costs the daemon a current-state read once it is proven idle past the threshold, and at most one such read per `FM_STALE_ESCALATE_SECS` window while it stays absorbed.
That is one read for a non-claude harness or for a run that is still advancing, and two when a claude pane's run is not advancing and it falls through to the usage-limit question on the same pass.
The second read is bounded to at most one per stale marker and does not repeat while a pane stays absorbed, because the pass that reaches it is the pass that escalates and drops the marker.
That same window also gates the pane capture in front of the read, because an absorbed pane keeps its stale marker and so reaches the threshold on every housekeeping tick.
The cost falls on both transitions that gate sits in front of: a pane which resumes while absorbed, and a run which stops advancing while absorbed, are each noticed at the next gate opening rather than within a tick.
...
An absent `quota-axi` reports `MISSING: quota-axi (install: npm install -g quota-axi)`; firstmate cannot resolve a profile array until current quota output is available for every candidate, and a crewmate stalled on Claude Code's usage-limit prompt reports an unreadable window, which authorizes neither automatic recovery nor a settled wait.
