...
It reads the same `FM_WEDGE_DEMAND_INSPECT_COUNT` as a cap on one stale episode: after that many still-advancing rechecks the episode escalates as a possible wedge demanding deep inspection instead of absorbing again, so a crew halted behind a run step nobody can measure cannot stay quiet while the captain is away.
