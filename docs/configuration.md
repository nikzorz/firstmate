...
That same window also gates the pane capture in front of the read, because an absorbed pane keeps its stale marker and so reaches the threshold on every housekeeping tick; the cost is that a pane which resumes while absorbed is noticed up to one window late rather than within a tick.
