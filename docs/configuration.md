...
It holds for any posture that runs the away-mode daemon, away or quiet alike, because a live daemon plus `state/.afk` is the evidence it trusts.
A posture with no away-mode daemon (Pi, or away mode run by the supervision host under `config/supervision-host`) never arms it, so the machine can still sleep there.
...
With it present the same two signals also silence the terminal stale path: a stale pane whose crew's own last status line is that still-open `needs-decision` is absorbed on each new pane hash with no wedge timer, because the answer rather than the timer owns its next wake; with it absent that pane surfaces on each new hash as before.
...
## Still-advancing-run absorb

At the wedge threshold, after the declared-wait, worktree-write and dead-endpoint checks, the watcher asks the crew once more: a no-mistakes run step still advancing behind a confirmed-live endpoint restarts the window instead of escalating, because pane idleness is the expected shape of a long pipeline step, while a run that has stopped advancing reads `stalled` and escalates exactly as before.
The endpoint must be confirmed alive, so backends whose agent-state classifier cannot answer keep the unchanged schedule.
The absorb is bounded: a pane held by it re-surfaces for a recheck once per `FM_PAUSE_RESURFACE_SECS`, aged by the longer of the pane's true idle age (measured from its signature rather than the restarted timer) and the time since the absorb episode began or last rechecked, so a busy pane whose footer rewrites its signature every poll is still rechecked.
The run of rechecks is capped by `FM_WEDGE_DEMAND_INSPECT_COUNT`: the recheck reaching that number escalates as a possible wedge demanding deep inspection instead of absorbing again, so at the default of three the reader sees absorb, absorb, wedge.
That escalation clears the count but keeps the timer, so a crew that has since recovered is not left flagged while one that has not is absorbed again and earns another demand-inspection a cap of rechecks later.
The absorb also never takes the escalation that would carry `demand-deep-inspection` on the separate consecutive-escalation count, so that marker always reaches firstmate.
Away mode asks the same question on the same terms through the same `crew_run_step_advancing`, and applies the same cap within one stale episode; the daemon's escalation ends that episode, so a pane frozen for the whole away window gets one demand-inspection there and no repeat.
`FM_STALE_ESCALATE_SECS` is both the threshold at which the question is asked and the window that throttles it, so an absorbed pane costs at most one current-state read per window, on any harness.
A busy pane past `FM_BUSY_TURN_MAX_SECS` reaches the same wedge timer, so the same absorb holds it on the same terms.
Possible-wedge escalation reasons report the pane's true idle age, measured from its signature record, because every deferral restarts the timer.
The still-advancing-run absorb sections of `bin/fm-watch.sh` (`wedge_defer_advancing`) and `bin/fm-supervise-daemon.sh` own the full contract.
...
An absent or too-old `quota-axi` reports `MISSING: quota-axi (install: npm install -g quota-axi)`; firstmate cannot resolve a profile array without a compatible binary, and a crewmate stalled on Claude Code's usage-limit prompt then reports an unreadable window, which authorizes neither automatic recovery nor a settled wait.
...
On that signature only, `fm-fleet-sync.sh` retries the fetch with a bounded wait for the lock to self-clear, then removes the lock and retries once more only when it can prove the lock stale, the same recovery shape `fm-teardown.sh` runs once it has classified a worktree `index.lock` race.
...
FM_CREW_STATE_FORGE_PROBE=1   # 1 lets fm-crew-state.sh ask the forge whether what a monitoring ci step waits for can still arrive; anything else, or FM_CREW_STATE_NO_FORGE=1, switches that query off
...
FM_CLAUDE_LIMIT_QUOTA_TIMEOUT=15   # seconds allowed for the bounded, read-only quota-axi read behind that decision
FM_CLAUDE_LIMIT_RESET_GRACE_SECS=60   # seconds added to a reported window reset before the recorded wait's until time
...
FM_EXTRA_USAGE_EPISODE_SECS=18000   # seconds bin/fm-extra-usage.sh stays silent after stopping the fleet for extra usage
...
FM_WEDGE_DEMAND_INSPECT_COUNT=3    # consecutive repetitions on the same unchanged pane before demand-deep-inspection is added, applied to two separate runs: provably-working stale escalations, and the long-cadence rechecks the still-advancing-run absorb raises within one stale episode; the absorb never takes the escalation that would carry the marker, and the away-mode daemon applies the same cap to the same absorb on its own path; the default lives in bin/fm-classify-lib.sh (FM_WEDGE_DEMAND_INSPECT_COUNT_DEFAULT)
...
FM_TREEHOUSE_RETURN_LOCK_RETRIES=3        # retries after a treehouse return fails on a transient git index.lock race
FM_TREEHOUSE_RETURN_LOCK_RETRY_WAIT_SECS=1 # seconds fm-teardown.sh waits before each of those retries
...
FM_SEND_SETTLE=1        # seconds fm-send waits after a successful typed-plane submit; 0 disables, and an unusable value is reported on stderr and replaced by the default rather than changing what the send reports
...
A return can also fail with no git lock in evidence at all while the worktree is clean and an unchanged retry succeeds immediately.
Teardown classifies that arm from its own evidence too, gated on a proof re-derived at the moment of each retry: the worktree must be inspectable, carry no uncommitted change, and hold no commit missing from both every remote and the local default branch.
That proof is deliberately narrower than the pre-return landed-work check, and nothing is carried over from it.
