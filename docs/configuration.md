...
Unlike the claude usage-limit question beside it, that one is not pre-filtered on the recorded harness, because a run step belongs to no harness: a stale pane on any harness costs the daemon one current-state read once it is proven idle past the threshold, and at most one per `FM_STALE_ESCALATE_SECS` window while it stays absorbed.
