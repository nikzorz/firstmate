...
The verified adapter knowledge - busy signatures, interrupt and exit commands, skill-invocation syntax, and per-harness quirks - lives in [`.agents/skills/harness-adapters/`](../.agents/skills/harness-adapters/SKILL.md): the router in `SKILL.md` carries the cross-harness contract and the verified-harness list, and one [`references/<harness>.md`](../.agents/skills/harness-adapters/references/) file per harness carries that harness's own facts.
...
A send whose delivery could not be confirmed keeps the same marker but emits `NUDGE_SECONDMATES:` as `send unconfirmed:`, because that nudge may already have landed; [`bootstrap-diagnostics`](../.agents/skills/bootstrap-diagnostics/SKILL.md) owns how to handle both lines.
...
FM_SESSION_START_STATUS_NEWEST_BYTES=1000  # bytes kept from the newest status line before a visibly marked clip; a line only just past it prints whole instead, when the per-task budget still has room, because the marker would cost more than it hides
FM_SESSION_START_STATUS_OLDER_BYTES=200    # the same cap, and the same whole-line rule, for each older status line in the tail
FM_SESSION_START_STATUS_TASK_BYTES=2300    # total status-tail bytes per task; the budget is spent newest-first, so the oldest lines drop out past it
...
FM_WEDGE_DEMAND_INSPECT_COUNT=3    # consecutive repetitions on the same unchanged pane before demand-deep-inspection is added, applied to two separate runs: provably-working stale escalations, and the long-cadence rechecks the still-advancing-run absorb raises within one stale episode; the absorb stops one window before the escalation count so that escalation always fires, and the away-mode daemon applies the same cap to the same absorb on its own path
