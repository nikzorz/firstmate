...
It applies the same `FM_WEDGE_DEMAND_INSPECT_COUNT` cap too: after that many still-advancing rechecks in a row on one pane the daemon stops absorbing and escalates it as a possible wedge carrying the same demand-deep-inspection instruction, so a crew halted behind a run step nobody can measure cannot stay quiet while the captain is away.
...
FM_WEDGE_DEMAND_INSPECT_COUNT=3    # consecutive provably-working stale escalations on the same unchanged pane before demand-deep-inspection is added; also caps the still-advancing-run absorb in the watcher and the away-mode daemon, which stop one window before this count so that escalation always fires
