...
- Retiring a secondmate home clears each child's journal along with its other per-task records, with or without `--force` and whether or not that child still has a meta file, because a home occupying a treehouse slot is returned for reuse rather than deleted and a surviving journal would bind the next task that reuses that id.
